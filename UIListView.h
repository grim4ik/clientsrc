...
//new class
class CItemList : public CListView {
public:
    CItemList(CForm& frmOwn, int nCol) : CListView(frmOwn, nCol, CListView::eWindowTitle) {}

private:
    virtual CCompent* GetHintCompent(int x, int y) {
        if (!GetList()->InRect(x, y)) {
            return nullptr;
        }

        if (CItemRow* select = GetList()->GetSelectItem()) {
            SetHintItem(select->GetIndex(0));
        }
