...
void CFastCommand::Render()
{
	/*
	try{
		if ( _pCommand2 !=nullptr ) _pCommand2->Render( GetX()+3, GetY() );
...
: CCommandCompent(frmOwn), _pCommand(nullptr), evtBeforeAccept(nullptr), evtThrowItem(nullptr)
, evtUseCommand(nullptr), _pActive(nullptr), _IsShowActive(false), _eShowStyle(enumSmall)
{
	_SetSelf();
}
