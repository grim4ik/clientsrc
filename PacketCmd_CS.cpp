...
bool CS_Connect(dbc::cChar *hostname,dbc::uShort port,dbc::uLong timeout)
{
	...
	discordInit ();
	updateDiscordPresence ("Playing on Starlight Isle", "");
...
void CS_Disconnect(int reason)
{
	...
	isloaded = false;
	updateDiscordPresence("Login Section", "Ready to wipe all!");
	// clear maze portals
...
{
	g_stUISystem.m_sysProp.ApplyAudio();	//music fix
