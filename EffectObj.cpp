...
CMagicEff::CMagicEff()
{
	...
	_nRoll = 0;
	_fHei = 0;
	GroupEmission = nullptr;
	RenderUpdate = nullptr;
	_pDailTime = nullptr;
	_iDummy = -1;
	_nHeightOff = -1;
	_iIdxID = -1;
...
void	CMagicEff::FrameMove(DWORD dwDailTime)
{
	if(_bDail)
	{
		...
		{
			_bDail = false;
			Emission(_isID,_vsBegin.x < 0? nullptr : &_vsBegin,_vsEnd.x < 0? nullptr : &_vsEnd);
		}
		return;
	}
