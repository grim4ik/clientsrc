...
namespace GUI
{
	...
	bool CMakeEquipMgr::Init()
	{
		...
		labForgeRate = dynamic_cast<CLabel*>(frmMakeEquip->Find("labForgeRate"));
		if (!labForgeRate)
		{
			return Error(g_oLangRec.GetString(561),
				frmMakeEquip->GetName(),
	...
	long CMakeEquipMgr::CalMakeEquipMoney()
	{
		...
		{
		case MAKE_EQUIP_TYPE:
			//mothannakh @ combine rate client side, make sure to update it if updated lua side @
		{
			pItemCommand = dynamic_cast<CItemCommand*>(cmdItem[0]->GetCommand());
			iLevelPlusOne = pItemCommand->GetData().sEnergy[1] - 1;
			if (iLevelPlusOne < 0) iLevelPlusOne = 0;
			char* RefCombineRates[] = { "100%", "95%", "90%", "85%", "80%", "75%", "70%", "60%", "66%" };	//ref gems
			char* GemsCombineRates[] = { "100%", "90%", "80%", "70%", "60%", "50%", "40%", "30%", "20%" };	//general gems 
			 char buf[30] = "";
			 sprintf_s(buf, sizeof(buf), "Combine Rate: %s", (pItemCommand->GetItemInfo()->sType == 50? RefCombineRates[iLevelPlusOne]: GemsCombineRates[iLevelPlusOne]));
			 labForgeRate->SetCaption(buf);
