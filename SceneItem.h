...
public:
	...
	void	SetIsProtected(bool isProtected = false);
	bool	GetIsProtected() { return isProtected; };
	void	SetTicks(long startTick, long currTick, long endProtTick);
	void	TransformToUnprotectedItem();
protected:
...
protected:
	...
	DWORD			_dwForgeValue;
	char DropType{ 0 };//check what item dropped by monster/player etc @mothannakh
	bool isProtected;
	DWORD	startTick;
	DWORD	currTick;
	DWORD	protEndTick;
...
inline void CSceneItem::SetIsProtected(bool prot) {
	isProtected = prot;
}

inline void CSceneItem::SetTicks(long start, long cur, long endProt) {
	long diffBetweenStartAndCurrentTick = cur - start;

	currTick = CGameApp::GetCurTick();
	startTick = currTick - diffBetweenStartAndCurrentTick;
	protEndTick = endProt;
}