...
{
    if( !_pSkill )  LG( "error", "msgCSkillCommand::CSkillCommand(CSkillRecord * pSkill) pSkill is nullptr" );
...
{ 
	...
	{
	//	g_pGameApp->SysInfo( "Can't use skills while mount is in use." );
	//	g_pGameApp->ShowBottomText(0xC91E1E, "Can't use skills while mount is in use");
	//	g_pGameApp->MsgBox( "Please un-equip mount if you want to use skills" );
