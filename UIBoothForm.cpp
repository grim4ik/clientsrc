...
namespace GUI
{
	...
	bool CBoothMgr::Init() // 摆摊信息初始化
	{
		...
		//button offline stall @mothannakh

		btnOffLineStall = dynamic_cast<CTextButton*>(frmBooth->Find("btnOffLineStall"));
		if (!btnOffLineStall)
			return Error(g_oLangRec.GetString(446),
				frmBooth->GetName(), "btnOffLineStall");
		//end
	...
	//~ ------------------------------------------------------------------
	void CBoothMgr::ShowSetupBoothForm(int iLevel)  // Display settings stall interface
	{
		...
		//Obtain the number of stalls according to the level
		...
		//Set UI controls for the stall interface
		...
		btnOffLineStall->SetIsShow(false);
	...
	void CBoothMgr::ShowTradeBoothForm(DWORD dwOwnerId, const char* szBoothName, int nItemNum)
	{
		...
		// Set UI controls for the trading interface
		...
		if (!visitedStalls.count(dwOwnerId)) {
			visitedStalls.insert(dwOwnerId);
		}

		//g_pGameApp->SysInfo("test");
	...
	void CBoothMgr::SetupBoothSuccess()
	{
		...
		btnOffLineStall->SetIsShow(true);
		edtBoothName->SetIsEnabled(false);
		...
		int x, int y, DWORD dwKey)
	{
		...
		{	///Press the "Set up stall" key
			...
			//Check if the stall name contains illegal characters
			std::string sName(g_stUIBooth.edtBoothName->GetCaption());
			...
			// Fill in the information content
			mission::NET_STALL_ALLDATA netCreateBoothData;
			int iNum(0);	//Amount in the booth
			for (int i(0); i < g_stUIBooth.m_iBoothItemMaxNum; i++)
			{
				if (g_stUIBooth.m_kBoothItems[i])
				{
			...
			{
				//Send to create a stall agreement
		...
		//offline stall button
		else if (name == "btnOffLineStall")
		{
	...
	void CBoothMgr::_InquireSetupPushItemPriceEvent(CCompent* pSender, int nMsgType, int x, int y, DWORD dwKey)
	{
		...
		g_stUIBooth.m_pkCurrSetupBooth->iPrice = kItemPriceBox->GetNumber();	// Unit price entered by the player
	...
	bool CBoothMgr::PushToBooth(CGoodsGrid& rkDrag, CGoodsGrid& rkSelf, int nGridID, CCommandObj& rkItem)
	{
		...
		if (pkCha->getAttachID() == GetOwnerId())
		{	///The owner is currently opening the interface

			//Determine if an item is tradable
		...
		{	///If you are not the stall owner, you can consider the transaction as a transaction.
	...
	bool CBoothMgr::PopFromBooth(CGoodsGrid& rkDrag, CGoodsGrid& rkSelf, int nGridID, CCommandObj& rkItem)
	{
		...
		if (pkCha->getAttachID() == GetOwnerId())
		{	///The owner who is currently opening the interface is the stall owner.
