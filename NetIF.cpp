...
BOOL NetIF::HandlePacketMessage(DataSocket *datasock,LPRPACKET pk)
{
	...
	case CMD_MC_SetForgeRate: return SC_SetForgeRate(pk);
...
void NetIF::SendPacketMessage(LPWPACKET pk)
{
	...
	if(!m_connect.GetDatasock())
	{
		...
		{
			LG("error", "msgg_pGameApp->GetCurScene() Is nullptr, Can't Send Socket Message!\n");
			return;
		}
		if (!g_pGameApp->GetCurScene()->GetMainCha())
		{
			LG("error", "msgg_pGameApp->GetCurScene()->GetMainCha() Is nullptr, Can't Send Socket Message!\n");
			return;
		}
		LG("error", "msgClientSocket Is nullptr, Can't Send Socket Message!\n");
		return;
	}
