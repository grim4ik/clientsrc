...
CForm*          CLoginScene::frmKeyboard = nullptr;	// add by Philip.Wu  软键盘界面  2006-06-05
CForm*          CLoginScene::frmPathLogo = nullptr;

//CForm*			CLoginScene::frmHintLogo = nullptr;

//CImage*			CLoginScene::imgHintLogo = nullptr;

CCheckBox*      CLoginScene::chkID       = nullptr;

CList*			CLoginScene::lstRegion0;
CList*			CLoginScene::lstRegion1;
CListView*		CLoginScene::lvServer0;
CListView*		CLoginScene::lvServer1;

CEdit*			CLoginScene::edtID       = nullptr;
CEdit*          CLoginScene::edtPassword = nullptr;
CEdit*			CLoginScene::edtFocus    = nullptr;	// add by Philip.Wu  光标激活的编辑框  2006-06-06
CCheckBox*      CLoginScene::chkShift    = nullptr;	// add by Philip.Wu  软键盘上的 Shift  2006-06-09
...
void CLoginScene::ShowLoginForm()
{
	...
	//frmHintLogo->SetIsShow(true);


	//imgHintLogo->SetIsShow(true);
	//imgHintLogo->SetAlpha(0);
...
void CLoginScene::_FrameMove(DWORD dwTimeParam)
{
	...
	if (frmServer->GetIsShow())
	{
		...
		if ( !lvServer0->InRect(x, y) )
		{
			lvServer0->GetList()->GetItems()->GetSelect()->SetNoSelect();
		}
		if ( !lvServer1->InRect(x, y) )
		{
			lvServer1->GetList()->GetItems()->GetSelect()->SetNoSelect();
		}
	}

	if (frmRegion->GetIsShow())
	{
		if ( !lstRegion0->InRect(x, y) )
		{
			lstRegion0->GetItems()->GetSelect()->SetNoSelect();
		}
		if ( !lstRegion1->InRect(x, y) )
		{
			lstRegion1->GetItems()->GetSelect()->SetNoSelect();
		}
	}
        ...
        case Connection::CNST_FAILURE: 
            {
				...
				CForm* frm = CFormMgr::s_Mgr.Find("frmServer");
				if (frm)
					frm->Show();
				ShowKeyboard(false);
				g_pGameApp->MsgBox(g_oLangRec.GetString(169));
...
void CLoginScene::_Render()
{
	...
	if( IsLoad )
	{
		LoginPic.LoadImage( "texture/ui/custom/biglogo1.png", 402, 286, 0, 0, 0, 1.F, 1.F );
		IsLoad = true;
	}
	...
	/*if (frmHintLogo->GetIsShow())
	{
		CGraph* pGraph = nullptr;
		CItemRow* pRow = nullptr;
		static CGuiPic LogoHint;		

		if( pGraph )
		{
			LogoHint.LoadImage( "texture/ui/custom/biglogo1.png", 402, 286, 0, 0, 0, 1.F, 1.F );
			pGraph->GetImage()->SetAlpha( 255 );
			...
			{
			case 0: 
				sprintf( szBuf, "blyat", pRow->GetIndex(1)->GetString() );
				break;
			}		
...
void CLoginScene::_evtServerFrm(CCompent *pSender, int state, int x, int y, DWORD key)
{
	...
	{
		pSender->GetForm()->SetIsShow(false);
		int select = lvServer0->GetList()->GetItems()->GetSelect()->GetIndex();
		if (select != -1)
		{
			select = 2 * select;
		}
		...
		{
			select = lvServer1->GetList()->GetItems()->GetSelect()->GetIndex();
			if (select != 1)
			{
				select = 2 * select + 1;
			}
		}
		pkScene->m_iCurSelServerIndex = select;
		pSender->GetForm()->SetIsShow(false);
		if (frmAccount)
		{
...
void CLoginScene::_evtServerLDBDown(CGuiData *pSender, int x, int y, DWORD key)
{
	...
	if (pkServerList == lvServer0->GetList())
	{
		pkScene->SetCurSelServerIndex(pkServerList->GetItems()->GetSelect()->GetIndex() * 2);
	}
	else if (pkServerList == lvServer1->GetList())
	{
...
{
	lstRegion0->GetItems()->Clear();
	lstRegion1->GetItems()->Clear();

	CServerSet* server_set = CServerSet::I();
	for (int i = 0; i < server_set->m_nRegionCnt; i++)
	{
		if (i % 2 == 0)
			lstRegion0->Add(server_set->m_szRegionName[i]);
		else
			lstRegion1->Add(server_set->m_szRegionName[i]);
	}

	SetCurSelRegionIndex(0);
	CListItems* items = lstRegion0->GetItems();
	if (!items) return;
...
{
	lvServer0->GetList()->GetItems()->Clear();
	lvServer1->GetList()->GetItems()->Clear();

	for (int i = 0; i < GetCurServerGroupCnt(nRegionNo); i++)
	{
		CListView* server_group;
		if (i % 2 == 0)
		{
			server_group = lvServer0;
		}
		else
			server_group = lvServer1;

		CItemRow* item_row = server_group->AddItemRow();

		if (item_row)
		{
			CItem* v7 = new CItem(GetCurServerGroupName(nRegionNo, i), COLOR_BLACK);
			v7->SetColor(lvServer1->GetList()->GetFontColor());
	...
	CListItems *items = lvServer0->GetList()->GetItems();
	if (!items) return;
...
BOOL CLoginScene::_InitUI()
{
	...
	lvServer0 = dynamic_cast<CListView*>(frmServer->Find("lvServer0"));
	if (!lvServer0) return false;
	lvServer0->GetList()->evtListMouseDown = _evtServerLDBDown;


	lvServer1 = dynamic_cast<CListView*>(frmServer->Find("lvServer1"));
	if (!lvServer1) return false;
	lvServer1->GetList()->evtListMouseDown = _evtServerLDBDown;
	...
	lstRegion0 = dynamic_cast<CList*>(frmRegion->Find("lstRegion0"));
	if (!lstRegion0) return false;
	lstRegion0->evtListMouseDown = _evtRegionLDBDown;

	lstRegion1 = dynamic_cast<CList*>(frmRegion->Find("lstRegion1"));
	if (!lstRegion1) return false;
	lstRegion1->evtListMouseDown = _evtRegionLDBDown;
	...
	m_sSaveAccount = std::string(szName);
	edtID->SetCaption( m_sSaveAccount.c_str());
	...
	//imgHintLogo = (CImage*) frmHintLogo->Find("imgHintLogo");
	//if(!imgHintLogo) return false;
...
bool CLoginScene::_Bill()
{
    ...
    if (g_Config.m_IsBill)
	{
        ...
        {
            CBoxMgr::ShowMsgBox( nullptr, g_oLangRec.GetString(176), true );
        }
        else
        {
            ...
            {
                CBoxMgr::ShowMsgBox( nullptr, g_oLangRec.GetString(177), true );
            }
            else if(ret == "[Player_Failure]")
            {
                CBoxMgr::ShowMsgBox( nullptr, g_oLangRec.GetString(178), true );
            }
            else
            {
                CBoxMgr::ShowMsgBox( nullptr, "PAI: Unknown Error. !", true );
            }
        }
		return false;
	}
...
void CLoginScene::SaveUserName(CCheckBox& chkID, CEdit& edtID)
{
	...
	m_sSaveAccount = std::string(edtID.GetCaption());
...
void CLoginScene::_evtKeyboardFromMouseEvent(CCompent *pSender, int nMsgType, int x, int y, DWORD dwKey)
{
	...
	std::string strText = edtFocus->GetCaption();
...
/*void CLoginScene::ShowHintLogo(int isShow)
{
	frmHintLogo->SetIsShow(isShow);

	imgHintLogo->SetIsShow(isShow);
}*/
