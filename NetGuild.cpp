...
/*void NetMC_LISTRANK_BEGIN(uChar gldtype)
{
	CGuildListMgr::ResetAll();
	CGuildListMgr::SetType(CGuildData::ranktype);
	
}

void NetMC_LISTRANK_END()
{
	CUIGuildList::ResetOrder();
	CGuildListMgr::SortGuildsByName();
	CUIGuildList::ShowGuildList();
...
void NetMC_LISTRANK(uLong id,cChar *name,cChar *motto,cChar *leadername,uShort memtotal,LLong exp)
{
	CGuildListMgr::AddGuild(new CGuildListData(id,name,motto,leadername,memtotal,exp));
}*/
...
void NetMC_LISTTRYPLAYER_BEGIN(uLong gldid,cChar *gldname,cChar *motto,char type,char stat,cChar *ldrname,uShort memnum,uShort maxmem,LLong money,LLong exp,uLong remain)
{
	...
	/*switch (type)
	{
	case 0: data.guild_type  = CGuildData::navy; break;
	case 1: data.guild_type  = CGuildData::pirate; break;
	default: data.guild_type = CGuildData::ranktype; break;
	}*/
...
{	
	...
	std::string str=chaname;
	if (strlen(motto)>0)
	{
		str+="("+std::string(motto)+")";
	}
