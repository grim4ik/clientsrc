	...
	if (fFile == nullptr)
	{
		if (CreateFile(ptcsFileName) == 0)
		{
			if (!bSilence)
			{
				...
				MessageBox(nullptr, tcsPrint, _TEXT(g_oLangRec.GetString(25)), 0);
			}
			lRet = 0;
			goto end;
		}
...
long CSceneObjFile::ConvertObjFileVer(_TCHAR *ptcsFileName, bool bBackUp) // �Ӱ汾500ת����600(����ľ�������ת�������section���������)
{
	long			lRet = 2;
	...
	MessageBox(nullptr, tcsPrint, _TEXT(g_oLangRec.GetString(361)), 0);
	...
	if (lRet == 2)
	{
		...
		MessageBox(nullptr, tcsPrint, _TEXT(g_oLangRec.GetString(363)), 0);
	}
	if (fFileOld)
		fclose(fFileOld);
	if (fFileNew)
...
long CSceneObjFile::WriteSectionObjInfo(int nSectionNO, SSceneObjInfo *SSceneObj, long lSectionObjNum)
{
	...
	{
		MessageBox(nullptr, _TEXT(g_oLangRec.GetString(365)), _TEXT(g_oLangRec.GetString(25)), 0);
	}
	//
...
long CSceneObjFile::TrimDirectory(_TCHAR *ptcsDirectory, bool bBackUp)
{
	...
	if (IDYES != MessageBox(nullptr, tcsPrint, _TEXT(g_oLangRec.GetString(361)), MB_YESNO))
		return 1;
