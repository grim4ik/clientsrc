...
{		
	...
	while (!feof(fp))
	{
		...
		else if (strcmp(aChunkType, "data") == 0)
		{
			...
			if (mSourceSounds[theSfxID]->Unlock(lpvPtr, dwBytes, nullptr, NULL) != DS_OK)
				return false;
...
bool DSoundManager::LoadSound(int theSfxID, const std::string& theFilename)
{
...
bool DSoundManager::WriteWAV(unsigned int theSfxID, const std::string& theFilename, const std::string& theDepFile)
{
	...
	if (fp <= 0)
	{
		mSourceSounds[theSfxID]->Unlock(lpvPtr, dwBytes, nullptr, NULL);
		return false;
	}	
