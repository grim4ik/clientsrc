...
void CWaitMoveState::ChaRun()
{
	...
	if(CGameApp::GetFrameFPS() == 30)
		_pCha->SetPoseVelocity( (float)_pCha->getMoveSpeed() / 650.0f ); //670.0f
	else
		_pCha->SetPoseVelocity( (float)_pCha->getMoveSpeed() / 630.0f ); //800.0f
    _pCha->RefreshItem();
}
