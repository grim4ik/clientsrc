...
: CStateSynchro(), _isTargetDied(false), _isDoubleAttack(false), _pTarget(nullptr), _pAttack(nullptr)
, _IsMiss(false), _pSkill(nullptr), _isBeatBack(false), _pRepSynchro(nullptr)
{
}
...
void CAttackEffect::ChaDied( CCharacter* pTarget, CCharacter* pAttack )
{
	...
	/*
	if (pTarget->getGameAttr()->get(ATTR_HP) > 0)
	{
		pTarget->getGameAttr()->set(ATTR_HP, pTarget->getGameAttr()->get(ATTR_HP));
		return;
	}
	*/
...
void CAttackEffect::ExecHarm( CSizeArray<stEffect>& Value, CCharacter* pTarget, CCharacter* pAttack )
{
	...
	if( Change.GetChangeBitFlag(ATTR_CEXP) && pTarget==g_stUIBoat.GetHuman() )
	{
		...
		//g_stUIStart.RefreshMainLifeExperience( );
	}
...
void CAttribSynchro::_Exec()
{
	...
	//	g_stUIStart.RefreshMainLifeExperience( pCha->getGameAttr()->get(ATTR_CLIFEEXP), pCha->getGameAttr()->get(ATTR_CLV_LIFEEXP) );
	...
	if( Change.GetChangeBitFlag(ATTR_LV) )
	{
		...
		if( !pCha->IsBoat() )
		{
			...
			{
				if( enumATTRSYN_INIT!=_nType && pCha->IsPlayer() )
				{
					pCha->SelfEffect( 132, -1 );
				}
			}
