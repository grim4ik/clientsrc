...
bool CTradeMgr::Init()
{
	...
	frmPlayertrade->evtMouseDragEnd = _OnDragTrade;
	// �����Լ�װ����
...
void CTradeMgr::ShowCharTrade( BYTE byType, DWORD dwAcceptID, DWORD dwRequestID )
{
	...
	std::string strPlayTradeLabName("");			//labPlayertradeName�ؼ�Caption
...
void  CTradeMgr::DragItemToTrade(DWORD dwCharID,USHORT sItemID,BYTE byIndex,BYTE byCount,BYTE byItemIndex, SItemGrid* pGrid, const NET_CHARTRADE_BOATDATA* const pBoat )
{
	...
	if( !pItem )
	{
		LG( "protocol", "GetItemRecordInfo return nullptr" );
		return;
	}
...
void CTradeMgr::_OnDragTrade(CGuiData *pSender, int x, int y, DWORD key)
{
	if (g_stUITrade.frmPlayertrade->GetTop()<=0)
	{
		g_stUITrade.frmPlayertrade->SetPos(g_stUITrade.frmPlayertrade->GetLeft(), 0);
		g_stUITrade.frmPlayertrade->Refresh();
	}

	if (g_stUITrade.frmPlayertrade->GetLeft()<=0)
	{
		g_stUITrade.frmPlayertrade->SetPos(0, g_stUITrade.frmPlayertrade->GetTop());
		g_stUITrade.frmPlayertrade->Refresh();
	}

	if (g_stUITrade.frmPlayertrade->GetBottom()>=GetRender().GetScreenHeight())
	{
		g_stUITrade.frmPlayertrade->SetPos( g_stUITrade.frmPlayertrade->GetLeft(),  GetRender().GetScreenHeight() - g_stUITrade.frmPlayertrade->GetHeight() );
		g_stUITrade.frmPlayertrade->Refresh();
	}

	if (g_stUITrade.frmPlayertrade->GetRight()>=GetRender().GetScreenWidth())
	{
		g_stUITrade.frmPlayertrade->SetPos(GetRender().GetScreenWidth() - g_stUITrade.frmPlayertrade->GetWidth(), g_stUITrade.frmPlayertrade->GetTop());
		g_stUITrade.frmPlayertrade->Refresh();
	}
}
