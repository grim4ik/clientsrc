...
int CCharacterModel::LoadPart(DWORD part_id, const char* file)
{
    ...
    if(_ModelType == MODEL_CHARACTER)
    {
        ...
        case VST_PB1U4NT0_LD:
            ...
            break;
        default:
            MessageBox(nullptr,"Crash, call jack", 0, 0);
        }
...
void CCharacterModel::PlayPose( DWORD pose, DWORD type, int time, int fps, DWORD blend_flag, DWORD blend_src_num, float velocityMultiplier)
{
    DWORD p = 0;
    _PoseType = pose;
    ...
    if(time > 0)
    {
		//here FPSModeSpeed		@mothannakh for 60fps speed up model animation 
		if(Is60FPSMode)
		{
			float v = 2000.0f/ (float)fps / ( (float)time / (float)(pi->end - pi->start) );
			velocity = v;
		}else
		{
    ...
    {
		if(Is60FPSMode)	//@mothannakh
		{
			float v = 60.0f / (float)fps;
			velocity = v;
		}else
		{
...
CSceneItem* CCharacterModel::DetachItem( CSceneItem* obj )
{
    ...
    for( DWORD i = 0; i < LINK_ID_NUM; i++ )
    {
        if( _LinkItem[i] && _LinkItem[i] == obj )
        {
			...
			// added by clp
			_LinkItem[i]->setParentCharacter( nullptr );
