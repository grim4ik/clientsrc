...
//char chap_key[12] = { 0x08, 0x17, 0x15, 0x20, 0x10, 0x03, 0x99, 0xAA, 0x7A, 0xA7, 0x09, 0x11 };
...
/*BOOL SC_ShowRanking(LPRPACKET pk){
	...
	for(uChar i =0;i<l_num;++i)
	{
		...
		NetMC_LISTRANK(l_id,l_name,l_motto,l_leadername,l_memtotal,l_exp);
	}
	NetMC_LISTRANK_END();
	return TRUE;
}*/

//void FormirateCryptChapStr(NetIF * pNet) {
	//encrypt_B(pNet->m_chapstrCrypt, sizeof(pNet->m_chapstrCrypt), chap_key, sizeof(chap_key));
//}

BOOL	SC_CHARSTR(LPRPACKET pk)
{T_B
	strncpy_s(g_NetIF->m_chapstr, sizeof(g_NetIF->m_chapstr), pk.ReadString(), _TRUNCATE);
	//memcpy(g_NetIF->m_chapstrCrypt, g_NetIF->m_chapstr, sizeof(g_NetIF->m_chapstrCrypt));
	// set clock time from m_chapstr @mothannakh
	struct std::tm time_set {}, time_get;
	//time_t timep;
	//time(&timep);
	__time64_t timep;
	_time64(&timep);
	//time_get = localtime(&timep);
	localtime_s(std::addressof(time_get), std::addressof(timep));
	int hh, mm, ss, dd, mth, mss;
	auto ret = sscanf_s(g_NetIF->m_chapstr, "[%d-%d %d:%d:%d:%d]", &mth, &dd, &hh, &mm, &ss, &mss);
	time_set.tm_year = time_get.tm_year;
	time_set.tm_mon = mth - 1;
	time_set.tm_mday = dd;
	time_set.tm_hour = hh;
	time_set.tm_min = mm;
	time_set.tm_sec = ss;
	time_set.tm_isdst = time_get.tm_isdst;
	g_NetIF->ClockTicks = difftime(mktime(&time_set), timep);

	//FormirateCryptChapStr(g_NetIF);
...
{T_B
	...
	}else			//�ɹ�
	{
		...
		//if(strlen(g_Config.m_szMD5Pass) == 32) // �����ļ���ֱ������MD5ֵ������?, ���������ұ���Ĵ���??
		//{
		//	l_chap.m_strPwdDig = g_Config.m_szMD5Pass; // ��;�޸����������MD5ֵ
		//}
		...
		//memset(g_NetIF->m_chapstrCrypt,0,sizeof(g_NetIF->m_chapstrCrypt));
		...
		// ������Ҽ���߳�  add by Philip.Wu  2006-07-10
...
BOOL SC_EnterMap(LPRPACKET pk)
{
	...
	{
		ofstream excp;
		excp.open("log\\SC_Map.txt", ios_base::app);
		excp << exc.what() << std::endl;
		excp.close();
		return FALSE;
	}
}
...
{T_B
	...
	SCreateInfo.isProtected = pk.ReadLong();
	SCreateInfo.startTick = pk.ReadLong();
	SCreateInfo.curTick = pk.ReadLong();
	SCreateInfo.endProtTick = pk.ReadLong();
		...
		case	enumACTION_SKILL_TAR:
			{
				...
				//if (pk.ReadChar())
				SSkillInfo.hassSrc = pk.ReadChar() ? true : false;
				if (SSkillInfo.hassSrc) 
				{
					// ����Դ״̬
...
{T_B
	...
	USHORT sLen = 0;
	const char* pszDesp = packet.ReadString();
	if( pszDesp == nullptr ) return FALSE;
	...
	case CMD_MC_CHARTRADE_ITEM:
		{
			...
			{
				MessageBox( nullptr, g_oLangRec.GetString(303), g_oLangRec.GetString(25), MB_OK );
				return FALSE;
			}
...
BOOL	SC_SetForgeRate(LPRPACKET pk)
{
T_B
		char szData[1024];
	long  type = pk.ReadLong();
	strncpy(szData, pk.ReadString(), 1024 - 1);
	if (type == enum_Forge)	//forge 
	{
		g_stUIForge.SetForgeRate(szData);
	}
	else	//ci red team//ci green team
	{
		g_stUIMap.SetCiPanel(type,szData);
	}
	//g_stUIMakeEquip.SetCombineRate(szData);
T_E
}
...
{T_B
	...
	SCreateInfo.szCommName = (char*)pk.ReadString();
	SCreateInfo.chGMLv		= pk.ReadChar();
	SCreateInfo.lHandle		= pk.ReadLong();
	SCreateInfo.chCtrlType	= pk.ReadChar();
	SCreateInfo.szName = (char *)pk.ReadString();
	SCreateInfo.strMottoName = static_cast<std::string>(pk.ReadString());
	SCreateInfo.sIcon       = pk.ReadShort();
	SCreateInfo.lGuildID = pk.ReadLong();
	SCreateInfo.strGuildName = static_cast<std::string>(pk.ReadString());
	SCreateInfo.strGuildMotto = static_cast<std::string>(pk.ReadString());
	SCreateInfo.chGuildPermission = pk.ReadLong();
	SCreateInfo.lGuildCircleColour = pk.ReadLong();
	SCreateInfo.chGuildIcon = pk.ReadChar();
	SCreateInfo.strStallName = static_cast<std::string>(pk.ReadString());
	//check if using our server exes or not 
	auto validation = static_cast<std::string>(pk.ReadString());
	if (validation != "Sex")
	{
		//pewpew your client bish 
	...
	SCreateInfo.sAngle = pk.ReadShort();
	SCreateInfo.ulTLeaderID = pk.ReadLong();
...
{T_B
	...
	if(	g_yyy_add_lock_item_wait_return_state	)
	{
		...
		if(	success	)
		{
			...
			//g_pGameApp->MsgBox("�������߳ɹ���");
			g_pGameApp->SysInfo("Locking successful!");
		}
		else
		{
			//g_stUIEquip.CloseAllForm();
			//g_pGameApp->MsgBox("������߲��ɼ�����??");
...
// add by ning.yan ���߽����Ļ�Ӧ��Ϣ 2008-11-12 begin
	...
	if(pObj){
		...
		if(extraCount > 0){
			for(int index = 0;index<extraCount;index++){
				...
				const float offX = pk.ReadLong();
				const float offY = pk.ReadLong();
				float actionYaw = pk.ReadLong();
