...
void CTeam::ReadBlockedList()
{
	FILE* fp = fopen("user\\blockedUsers.dat", "rb");
	if (fp)
	{
		uint16_t numBlocked;
		fread(&numBlocked, sizeof(numBlocked), 1, fp);

		for(int i = 0; i < numBlocked; i++)
		{
			uint16_t lenName;
			fread(&lenName, sizeof(lenName), 1, fp);
...
void CTeam::WriteBlockedList()
{
	CreateDirectory("user", nullptr);
	FILE* fp = fopen("user\\blockedUsers.dat", "wb");
	if (fp)
	{
		uint16_t numBlocked = _member.size();
		fwrite(&numBlocked, sizeof(numBlocked), 1, fp);

		for (CMember *mem : _member)
		{
			uint16_t lenName = strlen(mem->GetName());
			fwrite(&lenName, sizeof(lenName), 1, fp);

			fwrite(mem->GetName(), lenName, 1, fp);
		}
