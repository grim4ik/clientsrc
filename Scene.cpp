...
CAniClock*	    CGameScene::_AniClock = nullptr;
...
: _pChaArray(nullptr),
 _pSceneObjArray(nullptr),
 _pSceneItemArray(nullptr),
 _pSceneLightArray(nullptr),
 _pTerrain(nullptr),
 _nChaCnt(0),
 _nSceneObjCnt(0),
 _nSceneItemCnt( 0 ),
 _pEffectArray(nullptr),
 _nEffCnt(0),
 _nSceneLightCnt(0),
 _bShowTerrain(FALSE),
 _bShowSceneObj(TRUE),
...
CEffectObj*	CGameScene::GetFirstInvalidEffObj()
{
	...
	if(_pEffectArray==nullptr) return nullptr;
...
{
	if(nArrayID >= _nEffCnt || nArrayID < 0) return nullptr;
...
CCharacter* CGameScene::_GetFirstInvalidCha()
{
	...
	if(_pChaArray==nullptr) return nullptr;
...
{
	if(_pSceneObjArray==nullptr) return nullptr;
	int i;
...
{
	if(nArrayID >= _nChaCnt || nArrayID < 0) return nullptr;
	return &_pChaArray[nArrayID];
}

CSceneObj*	CGameScene::GetSceneObj(int nArrayID)
{
	if(nArrayID >= _nSceneObjCnt || nArrayID < 0) return nullptr;
...
#if 1
    std::list<int>::iterator it = _SceneObjIdx[SCENEOBJ_TYPE_POSE].begin();
    std::list<int>::iterator it_end = _SceneObjIdx[SCENEOBJ_TYPE_POSE].end();
...
bool CGameScene::_CreateMemory()
{
	...
	_nSceneItemCnt = 0;
	_nEffCnt = 0 ;
	if( _stInit.nMaxEff > 0 ) g_stUISystem.m_sysProp.m_videoProp.nResolution ? _CreateEffectArray(_stInit.nMaxEff*2) : _CreateEffectArray(_stInit.nMaxEff);
	if( _stInit.nMaxCha > 0 ) g_stUISystem.m_sysProp.m_videoProp.nResolution ? _CreateChaArray(_stInit.nMaxCha*2) : _CreateChaArray(_stInit.nMaxCha);
	_CreateSceneObjArray(1000);

	if( _stInit.nMaxItem > 0 ) g_stUISystem.m_sysProp.m_videoProp.nResolution ? _CreateSceneItemArray(_stInit.nMaxItem*2) : _CreateSceneItemArray(_stInit.nMaxItem);
...
{
	if(nArrayID >= _nShadeCnt || nArrayID < 0) return nullptr;
	return &_pShadeArray[nArrayID];
}
CShadeEff*	CGameScene::GetFirstInvalidShadeObj()
{
	if(_pShadeArray==nullptr) return nullptr;
...
{    
	...
	if( !pMapInfo ) 
	{
		LG( "switchmap", "msgCGameScene::SwitchMap - GetMapInfo(%d) return nullptr", nMapID );
		return false;
	}
...
{
    std::list<int>::iterator it = _SceneObjIdx[SCENEOBJ_TYPE_POINTLIGHT].begin();
    std::list<int>::iterator it_end = _SceneObjIdx[SCENEOBJ_TYPE_POINTLIGHT].end();
...
CAniClock* CGameScene::AddAniClock()
{
    ...
    LG( "AddAniClock", "msgCGameScene::AddAniClock return nullptr" );
    return nullptr;
}
