...
_pAniClock(nullptr), _pSkill(nullptr), _dwPlayTime(0),_canDrag(true)
{
    if( !_pItem )  LG( "error", "msgCItemCommand::CItemCommand(CItemRecord* pItem) pItem is nullptr" );
...
void CItemCommand::SaleRender( int x, int y, int nWidth, int nHeight )
{
	...
	if( sType == 49 || sType == 50){
		...
		auto GemLevel = item.sEnergy[1];
		_snprintf_s(buf, _TRUNCATE, "Lv%d.", GemLevel == 0 ? 1 : GemLevel);
		static int w, h;
	...
	}else if( sType == 90 )
	{
		static SItemHint item;
		static int w, h;
		sprintf( buf, "Mount", item.sEnergy[1]);
        CGuiFont::s_Font.GetSize( buf, w, h );
...
void CItemCommand::AddHint( int x, int y ){
	...
	if( isWeapon || isDefenceType || isJewelery || isEquip ){
		...
		PushHint( buf, DWORD(0xFFA723FF), 5, 1, -1, true, -16777216 );

		if( _pItem->lID == 1034 ){
			sprintf(buf, g_oLangRec.GetString(862), _ItemData.sEndure[0] * 10 - 1000, _ItemData.sEndure[1] * 10 - 1000);// ������㷽ʽ�?�ʲ߻�
			PushHint( buf, COLOR_WHITE, 5, 1 );
			return;
		}
		...
		short levelRequirement{ (_ItemData.sNeedLv == 0) ? _pItem->sNeedLv : _ItemData.sNeedLv };
	...
	else if( _pItem->sType==29 )    // shell
	{
		...
		DWORD CoralColor = COLOR_GREEN;
		int CoralCurValue = (_ItemData.sEnergy[0] * 100 / _ItemData.sEnergy[1]) ;
		if (CoralCurValue >= 21 && CoralCurValue <= 60)
		{
			CoralColor = ADVANCED_COLOR;
		}
		else if (CoralCurValue <= 20)
		{
			CoralColor = COLOR_RED;
		}
		else
		{
			CoralColor = COLOR_GREEN;
		}
        PushHint( buf , CoralColor);
	...
	else if( _pItem->sType == 46 )	// ����֤֮
	{
		...
		std::string show_text[] = { g_oLangRec.GetString(650), g_oLangRec.GetString(651), g_oLangRec.GetString(652), g_oLangRec.GetString(653), g_oLangRec.GetString(654) };
		int value = 0;
	...
	if( _hints.GetCount()>0 && (_pItem->sType<=27 || _pItem->sType==44) && _pItem->sType!=18 && _pItem->sType!=19 && _pItem->sType!=21) 
	{
		// �������ֵ�İ�λ�������?,ʮλ�����´�������ǰ׺
		char szBuf[16] = { 0 };
		sprintf( szBuf, "%09d", _ItemData.sEnergy[1]/10 );		// ��ʮ����Ϊ����������ֵΪ4λ��,�ȴ��Ժ���?��λ
	...
	PUSH_HINT("Trade",-1, _pItem->chIsTrade != 0 ? COLOR_GREEN : COLOR_RED);
	PUSH_HINT("Throw", -1, _pItem->chIsThrow != 0 ? COLOR_GREEN : COLOR_RED);
...
//extern	bool	g_yyy_add_lock_item_wait_return_state;		//	�Ѿ�����������Ϣ���ȴ������������������״�?��
	...
	else	if(	_wait_select_lock_item_state_	&&	_ItemData.sID	!=	5939	)
	{
		...
		if( pOwn )
		{
			if( pOwn==g_stUIEquip.GetGoodsGrid() )
			{
				...
				// add by ning.yan 2008-11-10 �������������������� begin
				if(_ItemData.dwDBID)
				{
				...
				else // end
				{
					...
					//delete by ning.yan ʹ��ʼ����ʾ����Ϸ�������� 2008-11-17 begin
...
bool CItemCommand::UseCommand(bool isRightClick)
{
  ...
  /* CGoodsGrid* pOwn = dynamic_cast<CGoodsGrid*>(GetParent());
	...
	/*	if(_ItemData.sID == 15044 && _ItemData.dwDBID == 0){
		...
		if( _pItem->lID == 15046  || _pItem->lID == 15045  || _pItem->sType == 66 || ( 864 <= _pItem->lID && _pItem->lID <= 866 ) )	// ���Ӽ���״̬  add by Philip.Wu  2006-06-20
		{
			CActor* pActor = CGameScene::GetMainCha()->GetActor();
