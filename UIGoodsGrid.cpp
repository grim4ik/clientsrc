...
, _nRow(0), _nCol(0), _nMaxNum(0), _pItems(nullptr)
, _nFirst(0), _nLast(0), _nTmpIndex(0), _eShowStyle(enumSmall)
, evtThrowItem(nullptr), evtSwapItem(nullptr), evtUseCommand(nullptr), evtBeforeAccept(nullptr), evtRMouseEvent(nullptr)
...
extern	bool	_wait_select_lock_item_state_;	//	The user has used the lock item and is waiting to select the locked item.
//	2008-9-17	yangyinyu	add	end!

bool CGoodsGrid::MouseRun(int x, int y, DWORD key) {
/*	if( !IsNormal() ){
		if(key & Mouse_RDown || key & Mouse_LDown){
			const auto item = dynamic_cast<CItemCommand*>(_pItems[_nTmpIndex]);
			if (item) {
				const CItemRecord* itemInfo = item->GetItemInfo();
				if (itemInfo->lID == 6888) {
	...
	if (_nTmpIndex != -1 && _pItems[_nTmpIndex]) {
		auto item = dynamic_cast<CItemCommand*>(_pItems[_nTmpIndex]);
		if (item && !item->GetCanDrag()) {
			return false;
		}
	}
...
int CGoodsGrid::_GetHitItem( int x, int y )
{
	...
	if (_nTmpIndex < 0 || _nTmpIndex >= _nLast)
	{
		_nTmpIndex = -1;
		return _nTmpIndex;
	}
