...
const int	AutoMountHeight = 75;	//for auto mount height @mothannakh
class CShadeEff;
class CEffectObj;
class CGameScene;
class CActor;
...
public: // �����ǵļ������?
	...
	//static void				SetIsShowGlows(bool v) { _ShowGlows = v; }
	static void				SetIsShowWings(bool v) { _ShowWings = v; }
	static void				SetIsShowPets(bool v) { _ShowPets = v; }
	//static void				SetIsShowMounts(bool v) { _ShowMounts = v; }
...
public:
	...
	CEffectObj* SkyEffect(int nEffectID, int nBeginDummy = 2, int nItemDummy = 0, int nSpeed = 400, D3DXVECTOR3* pTarget = nullptr, int nTargetChaID = -1, CSkillRecord* pSkill = nullptr);
...
public:	// Ӧ�ò�
	...
	//@shortcuts for monster info@mothannakh
	long			GetDef() { return _Attr.get(ATTR_BDEF); }
	long			GetPR() { return _Attr.get(ATTR_BPDEF); }
	long			GetMaxSP() { return _Attr.get(ATTR_BMXSP); }
	long			GetDodge() { return _Attr.get(ATTR_BFLEE); }
	long			GetHitRate() { return _Attr.get(ATTR_BHIT); }
	long			GetMinAttk() { return _Attr.get(ATTR_BMNATK); }
	long			GetMaxAttk() { return _Attr.get(ATTR_BMXATK); }
	//end of edit 
...
public:
	...
	void            UpdateTileColor(int currenthout);
	char* getLogName();
...
private:
	...
	//static	bool	_ShowGlows;
	static  bool	_ShowWings;
	static  bool	_ShowPets;
	//static  bool	_ShowMounts;
...
public:
	...
	void EquipMount(MountInfo* MountInfo);
    void RemoveMount();
    bool IsMounted() const { return pMount != nullptr; }
    bool IsMount() const { return pMountOwner != nullptr; }

protected:
	CCharacter *pMountOwner;
	int mParentBoneID;
	CCharacter* pMount;
	int MountID;	
    int MountBone;
...
public:
	void SetMountBone   (int    SMountBone)     { MountBone     = SMountBone; if (pMount) pMount->linkTo(this, MountBone); }
    void SetMountPose   (DWORD  SMountPose)     { MountPose     = SMountPose; if (pMount) RefreshItem(true); }
    void SetMountHeight (int    SMountHeight)   { MountHeight   = SMountHeight; }
    void SetMountX      (int    SMountX)        { MountX        = SMountX;      }
    void SetMountY      (int    SMountY)        { MountY        = SMountY;      }
    void SetMountScale  (int    SMountScale)    { MountScale    = SMountScale;  }
...
{
	if (nEquipPos < 0 || nEquipPos >= enumEQUIP_NUM) return nullptr;
