			    ...
			    if (infoex[i].GetType() == 0) // �������
			    {
					...
					else if( g_Config.m_bEnableLGMsg )// ���������Ӵ�������ѯ����ɾ��
					{
						_stprintf(tcsPrint,
							...
							infoex[i].GetID());
						if (IDYES == MessageBox(nullptr, tcsPrint, _TEXT(g_oLangRec.GetString(25)), MB_YESNO))
						{
...
void CGameApp::HandleKeyDown(DWORD dwKey)
{
    ...
    {
			if( g_Config.IsPower() || ( CGameScene::GetMainCha() && CGameScene::GetMainCha()->getGMLv() ) || g_Config.m_bEditor ) 
			{
			...
			if(IsMouseButtonPress(1))
			{
				...
				else
				{
					...
					if(pCha)
					{
						...
						if (g_stUISystem.m_sysProp.m_gameOption.bIsTop1CamMode)
	...
	switch (dwMsg) 
	{
		...
		case WM_SYSKEYDOWN:
		{
			...
			if( IsAltPress() )
			{
				...
				else if (dwParam1 == 'd' || dwParam1 == 'D' || dwParam1 == 'e' || dwParam1 == 'E')
				{
					CPage* tempInventory = dynamic_cast<CPage*>(g_stUIEquip.frmInv->Find("pgePublic"));
					if (!tempInventory) return false;
					if (dwParam1 == 'd' || dwParam1 == 'D')
					{
						

						if (g_stUIEquip.frmInv && (tempInventory->GetIndex() != 2|| !g_stUIEquip.frmInv->GetIsShow()))
						{
							tempInventory->SetIndex(2);
							g_stUIEquip.frmInv->Show();
						}
						else
						{
							if (g_stUIEquip.frmInv && tempInventory->GetIndex() == 2)
							{
								g_stUIEquip.frmInv->Hide();
								tempInventory->SetIndex(1);
							}
								
						}
					}
					else if (dwParam1 == 'e' || dwParam1 == 'E')
					{
						if (g_stUIEquip.frmInv && (tempInventory->GetIndex() !=0 || !g_stUIEquip.frmInv->GetIsShow()))
						{
							tempInventory->SetIndex(0);
							g_stUIEquip.frmInv->Show();
						}
						else
						{
							if (g_stUIEquip.frmInv && (tempInventory->GetIndex() == 0 || tempInventory->GetIndex() == 2))
							{
								g_stUIEquip.frmInv->Hide();
								tempInventory->SetIndex(1);
							}
				...
				if(CFormMgr::s_Mgr.GetEnableHotKey())	// Is the hotkey blocked
					CFormMgr::s_Mgr.OnHotKey( (char)dwParam1, 0 );
				else if(dwParam1 == 'p' || dwParam1 =='P')	// Special handling
					CFormMgr::s_Mgr.OnHotKey( (char)dwParam1, 0 );
			}
...
// ����ڷű༭����
//-----------------
const char* HandleMonsterCommand(std::string& strCmd, std::string &p1, std::string &p2)
{
	...
	if( strCmd=="load")
	{
		...
		while(!in.eof())
		{
			...
			std::string strRight = strList[1];
			Util_ResolveTextLine(strRight.c_str(), strList, 2, ')');
			std::string strValue = strList[0];							 // ȥ��������
...
{
	std::string strInput = pszCmd;
	std::string strList[80];
	int n = Util_ResolveTextLine(pszCmd, strList, 80, ' ');
	
	std::string strCmd = strList[0];
	std::string strRes = g_oLangRec.GetString(120);

	std::string p1 = strList[1];
	std::string p2 = strList[2];
	std::string p3 = strList[3];
	std::string p4 = strList[4];
...
T_B
	...
	{
		std::string hint = g_pGameApp->GetScriptMgr()->GetStoneHint( p1.c_str(), Str2Int( p2 ) );
