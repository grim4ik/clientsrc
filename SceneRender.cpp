...
#define OPEN_AMBIENT_ENV
    ...
    if(_bShowSceneObj)
    {
        ...
        std::list<int>::iterator it = _SceneObjIdx[0].begin();
        std::list<int>::iterator it_end = _SceneObjIdx[0].end();
        for(; it != it_end; ++it)
        {
            int nArrayID = (*it);
            CSceneObj *pObj = &_pSceneObjArray[nArrayID];
            if(pObj->IsValid() && pObj->IsHide()==FALSE)
		    {
				...
				if( _pTerrain )
				{
					...
					if(a != nArea)
					{
						...
						//mothannakh for daily light timer 
						DaylightByServerTime(env_light, env_color, pArea);
					}
				}
...
/////////
...

    {
		...
		int aReaMainCity = -1; //helper to change plaer skin if main city to our custom colors or keep as it  
		if( g_IsShowShop )
		{
			for(i = 0; i < _nChaCnt; i++) 
			{
				...
				if(cha_lgt)
				{
					...
					if (pArea) {
						if (pArea->chType)
							aReaMainCity = CurrentClientHour;
					}
					//@mothannakh daylight timer test 
					DaylightByServerTime2(env_light, db_color, c, pArea, tile, fLgt);
				}
				pCha->UpdateTileColor(aReaMainCity);
				// =========================================================
...
// DaylightByServerTime
void CGameScene::DaylightByServerTime(D3DLIGHTX& env_light, DWORD& env_color, CAreaInfo* pArea) {
	// just make sure we have valid data
	if (!pArea)
		return;
	MPDwordByte4 db_color{};
	db_color.d = pArea->dwLightColor;
	env_color = pArea->dwEnvColor;
	// we check if map is main city from areaset
	if (pArea->chType == 1) {
		float x = pArea->fLightDir[0], y = pArea->fLightDir[1], z = pArea->fLightDir[2]; // direction
		BYTE r = db_color.b[2], g = db_color.b[1], b = db_color.b[0];					 // rbg colors
		// filter scene color by current server time
		if (CurrentClientHour == 21 || CurrentClientHour == 20) {
			// sunset time?
			x = -3.0f;
			y = -1.0f;
			z = -1.0f;
		...
		else if (CurrentClientHour >= 4 && CurrentClientHour <= 6) { // early morning bit darky 
			x = -2.0f;
			y = 0.1f;
			z = 0.0f;
			// rbg colors
		...
		else if (CurrentClientHour == 22 || CurrentClientHour == 23) { // early morning bit darky 
			x = -3.0f;
			y = -1.1f;
			z = -1.0f;
			// rbg colors
		...
		// don't touch here
		env_light.Direction.x = x;
		env_light.Direction.y = y;
		env_light.Direction.z = z;
		D3DXVec3Normalize((D3DXVECTOR3*)&env_light.Direction, (D3DXVECTOR3*)&env_light.Direction);
		// main lights
		env_light.Diffuse.r = (float)(r / 255.0f);
		env_light.Diffuse.g = (float)(g / 255.0f);
		env_light.Diffuse.b = (float)(b / 255.0f);
		// otherwise render normal maps style if its not main city
	}
	else {
		// if not we use default colors
...
// DaylightByServerTime2 @mothannakh // didn't make changes here just default code to easier edit later
void CGameScene::DaylightByServerTime2(D3DLIGHTX& env_light, MPDwordByte4& db_color, D3DXCOLOR& c, CAreaInfo* pArea, MPTile*& tile, float& fLgt) {
	if (pArea) {
		env_light.Direction.x = pArea->fLightDir[0];
		env_light.Direction.y = pArea->fLightDir[1];
		env_light.Direction.z = pArea->fLightDir[2];
