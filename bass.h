...
#define BASS_TAG_OGG		2	// OGG comments : array of nullptr-terminated strings
#define BASS_TAG_HTTP		3	// HTTP headers : array of nullptr-terminated strings
#define BASS_TAG_ICY		4	// ICY headers : array of nullptr-terminated strings
#define BASS_TAG_META		5	// ICY metadata : nullptr-terminated string
...
RETURN : The text description of the device (nullptr=error) */
...
musvol : MOD music global volume level (nullptr=don't retrieve it)
samvol : Sample global volume level (nullptr=don't retrieve it)
strvol : Stream global volume level (nullptr=don't retrieve it) */
...
clsid  : Class identifier of the object to create (nullptr=use default) */
...
RETURN : A pointer to the requested interface (nullptr=error) */
...
distf  : Distance factor (nullptr=don't get it)
roolf  : Rolloff factor (nullptr=don't get it)
doppf  : Doppler factor (nullptr=don't get it) */
...
pos    : Position of the listener (nullptr=leave current)
vel    : Listener's velocity, used to calculate doppler effect (nullptr=leave current)
front  : Direction that listener's front is pointing (nullptr=leave current)
top    : Direction that listener's top is pointing (nullptr=leave current)
...
pos    : Position of the listener (nullptr=don't get it)
vel    : Listener's velocity (nullptr=don't get it)
front  : Direction that listener's front is pointing (nullptr=don't get it)
top    : Direction that listener's top is pointing (nullptr=don't get it)
...
env    : Reverb environment (nullptr=don't get it)
vol    : Reverb volume (nullptr=don't get it)
decay  : Decay duration (nullptr=don't get it)
damp   : The damping (nullptr=don't get it) */
...
RETURN : The loaded music's handle (nullptr=error) */
...
handle : Music handle
RETURN : The music's name (nullptr=error) */
...
RETURN : The loaded sample's handle (nullptr=error) */
...
RETURN : Memory location to write the sample's data (nullptr=error) */

HSAMPLE BASSDEF(BASS_SampleCreateDone)();
/* Finished creating a new sample.
RETURN : The new sample's handle (nullptr=error) */
...
RETURN : Handle of channel used to play the sample (nullptr=error) */
...
handle : Handle of sample to play
pos    : position of the sound (nullptr = x/y/z=0.0)
orient : orientation of the sound, this is irrelevant if it's an
         omnidirectional sound source (nullptr = x/y/z=0.0)
vel    : velocity of the sound (nullptr = x/y/z=0.0)
RETURN : Handle of channel used to play the sample (nullptr=error) */
...
RETURN : The created stream's handle (nullptr=error) */
...
flags  : Flags
save   : Filename to save the streamed file as locally (nullptr=don't save)
RETURN : The created stream's handle (nullptr=error) */
...
RETURN : Pointer to the tags (nullptr=error) */
...
drive  : The CD drive, for example: "d:" (nullptr=use default drive)
...
RETURN : ID string (nullptr=error) */
...
RETURN : The text description (nullptr=error) */
...
handle : Channel handle (HCHANNEL/HMUSIC/HSTREAM, or CDCHANNEL)
freq   : Pointer to store playback rate (nullptr=don't retrieve it)
volume : Pointer to store volume (nullptr=don't retrieve it)
pan    : Pointer to store pan position (nullptr=don't retrieve it) */
...
handle : Channel handle (HCHANNEL/HSTREAM/HMUSIC)
mode   : BASS_3DMODE_xxx mode (nullptr=don't retrieve it)
min    : minumum distance (nullptr=don't retrieve it)
max    : maximum distance (nullptr=don't retrieve it)
iangle : angle of inside projection cone (nullptr=don't retrieve it)
oangle : angle of outside projection cone (nullptr=don't retrieve it)
         NOTE: iangle & oangle must both be retrieved in a single call
outvol : delta-volume outside the projection cone (nullptr=don't retrieve it) */
...
handle : Channel handle (HCHANNEL/HSTREAM/HMUSIC)
pos    : position of the sound (nullptr=leave current)
orient : orientation of the sound, this is irrelevant if it's an
         omnidirectional sound source (nullptr=leave current)
vel    : velocity of the sound (nullptr=leave current) */
...
handle : Channel handle (HCHANNEL/HSTREAM/HMUSIC)
pos    : position of the sound (nullptr=don't retrieve it)
orient : orientation of the sound, this is irrelevant if it's an
         omnidirectional sound source (nullptr=don't retrieve it)
vel    : velocity of the sound (nullptr=don't retrieve it) */
...
user   : The 'user' value passed to the callback function
RETURN : Sync handle (nullptr=error) */
...
user   : The 'user' value passed to the callback function
RETURN : DSP handle (nullptr=error) */
...
RETURN : FX handle (nullptr=error) */
