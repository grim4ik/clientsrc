...
bool CEditor::_evtSetChaPosKeyChar(CGuiData *pSender, char& key)
{
    ...
    if( key ==',' || isdigit( key ) || key==VK_RETURN || key==VK_BACK || key=='-' )
    {
        if( key==VK_RETURN )
        {
            std::string str = g_stUIEditor.edtSetMainChaPos->GetCaption();
            size_t pos = str.find( ',' );
...
bool CEditor::_evtSetBrushHeightKeyChar(CGuiData *pSender, char& key)
{
    ...
    {		
        if( key==VK_RETURN )
        {
            std::string str = g_stUIEditor.edtSetBrushHeight->GetCaption();
            if( str.empty() )
            {
...
bool CEditor::_evtSetIslandIndex(CGuiData *pSender, char& key)
{
    if (key ==',' || isdigit( key ) || key==VK_RETURN || key==VK_BACK || key=='-')
    {
        if (key == VK_RETURN)
        {
            std::string str = g_stUIEditor.edtSetIslandIndex->GetCaption();
