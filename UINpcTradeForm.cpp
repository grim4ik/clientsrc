...
bool CNpcTradeMgr::Init()
{
	...
	frmNPCtrade->evtMouseDragEnd = _OnDragSell;
...
void CNpcTradeMgr::_OnDragSell(CGuiData *pSender, int x, int y, DWORD key)
{
	if (g_stUINpcTrade.frmNPCtrade->GetTop()<=0)
	{
		g_stUINpcTrade.frmNPCtrade->SetPos(g_stUINpcTrade.frmNPCtrade->GetLeft(), 0);
		g_stUINpcTrade.frmNPCtrade->Refresh();
	}

	if (g_stUINpcTrade.frmNPCtrade->GetLeft()<=0)
	{
		g_stUINpcTrade.frmNPCtrade->SetPos(0, g_stUINpcTrade.frmNPCtrade->GetTop());
		g_stUINpcTrade.frmNPCtrade->Refresh();
	}

	if (g_stUINpcTrade.frmNPCtrade->GetBottom()>=GetRender().GetScreenHeight())
	{
		g_stUINpcTrade.frmNPCtrade->SetPos( g_stUINpcTrade.frmNPCtrade->GetLeft(),  GetRender().GetScreenHeight() - g_stUINpcTrade.frmNPCtrade->GetHeight() );
		g_stUINpcTrade.frmNPCtrade->Refresh();
	}

	if (g_stUINpcTrade.frmNPCtrade->GetRight()>=GetRender().GetScreenWidth())
	{
		g_stUINpcTrade.frmNPCtrade->SetPos(GetRender().GetScreenWidth() - g_stUINpcTrade.frmNPCtrade->GetWidth(), g_stUINpcTrade.frmNPCtrade->GetTop());
		g_stUINpcTrade.frmNPCtrade->Refresh();
	}
}
