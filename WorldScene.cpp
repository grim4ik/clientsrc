...
void CWorldScene::_FrameMove( DWORD dwTimeParam )
{
			...
			if( _pMapInfo->IsShowSwitch )
			{
				...
				if( nArea!=_nOldMainChaInArea )
				{
					...
					if( pArea )
					{
						...
						static char disc[33];
						sprintf(disc,"in %s",pArea->szDataName);
						static char buffer[64] = {0};
						sprintf(buffer, "%s Lv%d %s", GetMainCha()->getHumanName(), GetMainCha()->getLv(),
							g_GetJobName((short)GetMainCha()->getGameAttr()->get(ATTR_JOB)));
 						updateDiscordPresence(buffer,disc);
					}
					else if( _pMapInfo )
					{
	...
	case GUI::enumMA_Skill:
		...
		else
		{
			...
			if( pSkill && pSkill->IsAttackArea() &&!CGameScene::GetMainCha()->GetIsOnMount() )	//mothannakh here to disable skills 
			{
	...
	case GUI::enumMA_None:
		...
		{	
			...
			else if( key & M_LDown )
			{
				...
				//small workaround do not allow player to send any path walks if already in stun @mothannakh
				CWorldScene* pScene = dynamic_cast<CWorldScene*>(CGameApp::GetCurScene());
				auto pcha = pScene->GetMainCha();
				if ( pcha) {
	...
	//if player have loot pet active 
	...
	//normal auto pick flow 
	if (_IsAutoPick) {
        if (static DWORD dwTime = 0; CGameApp::GetCurTick() > dwTime) {
            dwTime = CGameApp::GetCurTick() + 10000;
            [[maybe_unused]] auto unusedret = PickItem();
        }
    }
}
...
void CWorldScene::_SceneCursor()
{
	...
	if( g_pGameApp->IsMouseButtonPress(1) ){
		CCharacter* hoverCha = HitSelectCharacter( nMouseX, nMouseY);
		  if(hoverCha && !hoverCha->IsNPC() && hoverCha != pMain && hoverCha->getChaCtrlType() != enumCHACTRL_Mount){
			 RenderOnce = true ;
	...
	if( g_pGameApp->IsMouseButtonPress(2) ) // g_pGameApp->GetMainCam()->IsRotating()
	{
		g_pGameApp->GetCursor()->SetCursor( csNormalCursor );
		CGameScene::Set3DMouseState( enumClick );
	...
	bool isShowChaName = g_stUISystem.m_sysProp.m_gameOption.bIsChaNameShow;
	bool isShowMobName = g_stUISystem.m_sysProp.m_gameOption.bIsMobNameShow;
	bool isShiftPress = g_pGameApp->IsShiftPress();

		
    if( g_pGameApp->IsShiftPress() || isShowChaName || isShowMobName)
    {
		// ��סshift��ʾ���й�������
         for(int i = 0; i < _nChaCnt; i++)
        {
	...
	if( pSkill && pSkill->IsAttackArea() &&!pMain->GetIsOnMount())	//mothannakh here to disable skills 
	{
		// �ı����Ϊ��������
		CGameScene::Set3DMouseState( enumFollow );
		g_pGameApp->GetCursor()->SetCursor( csCircleCursor );
	...
	else
	{
		...
		// ���弼��,��⹥�������Ƿ���ȷ
		if( pSkill && !pMain->GetIsOnMount() ) //mothannakh here to disable skills 
		{
			CCursor::I()->SetFrame( CCursor::stSkillAttack );
			...
			if( g_pGameApp->IsCtrlPress() )
			{
				...
				if( pAttackCha && !pMain->GetIsOnMount() )	//mothannakh attack disable
				{
		...
		else
		{
			...
			{
				pAttackCha->GetHeadSay()->SetIsShowName( true );
				if (g_pGameApp->IsCtrlPress() && pMain != pAttackCha && !pMain->GetIsOnMount() )	//mothannakh attack disable
				{
				...
				else if( pAttackCha->IsNPC() )
				{
					...
					pAttackCha->SetColor( _bAttackRed, _bAttackGreen, _bAttackBlue );
					//rotate npcs to facing the player 
					if (g_NpcRotate->IsRotateActive())
					{
						if (g_NpcRotate->CanRotateIDs(pAttackCha->getMobID()) && g_NpcRotate->IsMatchDistance(pAttackCha->DistanceFrom(pMain)))	// 
						{
							pAttackCha->FaceTo(pMain->GetCurX(), pMain->GetCurY());
						}
				...
				else if( (pSkill=pMain->GetDefaultSkillInfo()) && g_SkillUse.IsAttack( pSkill, pMain, pAttackCha ) && !pMain->GetIsOnMount() && pAttackCha->getChaCtrlType() != enumCHACTRL_Mount && !g_stUISystem.m_sysProp.m_gameOption.bMeleeMode)
				{
					CCursor::I()->SetFrame( CCursor::stAttack ); 
				...
				else if( pAttackCha!=pMain && pAttackCha->IsPlayer() && !pMain->GetIsOnMount() )	//mothannakh 
				{
			...
			{
				if(pMain->GetIsOnMount()) return; 		// return false for seat on Chairs @mothannakh
			...
			{
					if(pMain->GetIsOnMount()) return; 		// return false for seat on Walls @mothannakh
...
void CWorldScene::_KeyDownEvent( int key )
{
	...
	if( g_pGameApp->IsCtrlPress() )
	{
		if( (key=='h' || key=='H') )
		{
			...
			g_pGameApp->SysInfo( _IsThrowItemHint ? "Open throw item hint" : "Close throw item hint" );
		}
		if( (key=='s' || key=='S') )
		...
		extern bool IsNumberTopBar; //top skill bar as number active
		if( (key=='b' || key=='B') )
		{
		...
		//top skill bar as number active
		if ((key == 't' || key == 'T'))
		{
			IsNumberTopBar = !IsNumberTopBar;
			g_pGameApp->ShowMidText(IsNumberTopBar ? "Top Skill Bar as Number :ON" : "Top Skill Bar as Number : Off");
			if(IsNumberTopBar) {
	...
	/*	if ((key == 'z' || key == 'Z'))
		{
			IsMeleAttack = !IsMeleAttack;
			g_pGameApp->SysInfo(IsMeleAttack ? "Mele Attack: Off" : "Mele Attack: On");
		}*/
		if( (key=='s' || key=='S') )
		{
...
{	
	...
	{
		/*
		LPDIRECT3DTEXTURE8		pCurSuf =nullptr;
		HRESULT hr;
