...
bFullScreen(false), nResolution(false), nTexture(0), nQuality(0),
...
*          -1 File is not exist.
*          -2 File is destroyed.
*			-3 Filename is nullptr or its length is zero.
*/
...
#define DEFAULT_NUM -2
	...
	iTemp = GetPrivateProfileInt("video", "nResolution", DEFAULT_NUM, szIniFileName);
	if (iTemp == DEFAULT_NUM)	return DEFAULT_NUM;
	m_videoProp.nResolution = int2bool(iTemp);
	...
	iTemp = GetPrivateProfileInt("gameOption", "camMode", DEFAULT_NUM, szIniFileName);
	if (iTemp == DEFAULT_NUM)	return DEFAULT_NUM;
	m_gameOption.bIsTop1CamMode = int2bool(iTemp);
	
	iTemp = GetPrivateProfileInt("gameOption", "state", DEFAULT_NUM, szIniFileName);
	if (iTemp == DEFAULT_NUM)	return DEFAULT_NUM;
	m_gameOption.bStateMode = int2bool(iTemp);

	iTemp = GetPrivateProfileInt("gameOption", "meleemode", DEFAULT_NUM, szIniFileName);
	if (iTemp == DEFAULT_NUM)	return DEFAULT_NUM;
	m_gameOption.bMeleeMode = int2bool(iTemp);

	iTemp = GetPrivateProfileInt("gameOption", "R.Click", DEFAULT_NUM, szIniFileName);
	if (iTemp == DEFAULT_NUM)	return DEFAULT_NUM;
	m_gameOption.bIsRightClick = int2bool(iTemp);

	iTemp = GetPrivateProfileInt("effects", "fairy", DEFAULT_NUM, szIniFileName);
	if (iTemp == DEFAULT_NUM)	return DEFAULT_NUM;
	m_gameOption.bIsFairyShow = int2bool(iTemp);
	
	iTemp = GetPrivateProfileInt("effects", "wing", DEFAULT_NUM, szIniFileName);
	if (iTemp == DEFAULT_NUM)	return DEFAULT_NUM;
	m_gameOption.bIsWingShow = int2bool(iTemp);
	
	iTemp = GetPrivateProfileInt("effects", "other", DEFAULT_NUM, szIniFileName);
	if (iTemp == DEFAULT_NUM)	return DEFAULT_NUM;
	m_gameOption.bIsOtherShow = int2bool(iTemp);
	
	iTemp = GetPrivateProfileInt("gameOption", "itemname", DEFAULT_NUM, szIniFileName);
	if (iTemp == DEFAULT_NUM)	return DEFAULT_NUM;
	m_gameOption.bIsItemNameShow = int2bool(iTemp);
	
	iTemp = GetPrivateProfileInt("gameOption", "chaname", DEFAULT_NUM, szIniFileName);
	if (iTemp == DEFAULT_NUM)	return DEFAULT_NUM;
	m_gameOption.bIsChaNameShow = int2bool(iTemp);

	iTemp = GetPrivateProfileInt("gameOption", "mobname", DEFAULT_NUM, szIniFileName);
	if (iTemp == DEFAULT_NUM)	return DEFAULT_NUM;
	m_gameOption.bIsMobNameShow = int2bool(iTemp);

	iTemp = GetPrivateProfileInt("gameOption", "frames", DEFAULT_NUM, szIniFileName);
	if (iTemp == DEFAULT_NUM)	return DEFAULT_NUM;
	m_gameOption.bIs60Fps = int2bool(iTemp);
...
#define ERROE_FILE_CANNT_CREAT -2
	...
	if (!WriteInteger("video", "fullScreen", bool2int(m_videoProp.bFullScreen), szIniFileName))
		return OTHER_ERROR;
	if (!WriteInteger("video", "nResolution", m_videoProp.nResolution, szIniFileName))
 		return OTHER_ERROR;
	...
	if (!WriteInteger("gameOption", "apparel", bool2int(m_gameOption.bAppMode), szIniFileName))
		return OTHER_ERROR;
	if (!WriteInteger("gameOption", "camMode", bool2int(m_gameOption.bIsTop1CamMode), szIniFileName))
		return OTHER_ERROR;
	if (!WriteInteger("gameOption", "state", bool2int(m_gameOption.bStateMode), szIniFileName))
		return OTHER_ERROR;
	if (!WriteInteger("effects", "fairy", bool2int(m_gameOption.bIsFairyShow), szIniFileName))
		return OTHER_ERROR;
	if (!WriteInteger("effects", "wing", bool2int(m_gameOption.bIsWingShow), szIniFileName))
		return OTHER_ERROR;
	if (!WriteInteger("effects", "other", bool2int(m_gameOption.bIsOtherShow), szIniFileName))
		return OTHER_ERROR;
	if (!WriteInteger("gameOption", "itemname", bool2int(m_gameOption.bIsItemNameShow), szIniFileName))
		return OTHER_ERROR;	
	if (!WriteInteger("gameOption", "chaname", bool2int(m_gameOption.bIsChaNameShow), szIniFileName))
		return OTHER_ERROR;	
	if (!WriteInteger("gameOption", "mobname", bool2int(m_gameOption.bIsMobNameShow), szIniFileName))
		return OTHER_ERROR;	
	if (!WriteInteger("gameOption", "frames", bool2int(m_gameOption.bIs60Fps), szIniFileName))
		return OTHER_ERROR;
...
void CSystemMgr::LoadCustomProp()
{
	...
	if (!m_isLoad)
	{
		if (m_sysProp.Load("user\\system.ini"))
		{
			...
			m_sysProp.m_videoProp.nResolution= true;//g_Render.GetScrWidth() > 1000 ? true : false;
			...
			m_sysProp.m_gameOption.bIsFairyShow = true;
			m_sysProp.m_gameOption.bIsWingShow = true;
			m_sysProp.m_gameOption.bIsOtherShow = true;
			m_sysProp.m_gameOption.bStateMode = true;
			
			m_sysProp.m_gameOption.bIsMobNameShow = false;		
			m_sysProp.m_gameOption.bIsItemNameShow = false;
			m_sysProp.m_gameOption.bIsChaNameShow = false;
			
			m_sysProp.m_gameOption.bIs60Fps = true;
	...
	Is60FPSMode = m_sysProp.m_gameOption.bIs60Fps;
}
bool CSystemMgr::Init()
{
	...
	frmSystem->evtMouseDragEnd = _OnDragMenu;
	...
	*/
	...
	cbxSize = ( CCheckGroup *)frmVideo->Find( "cbxSize" ); //��ͼ����
	if (! cbxSize )		return Error( g_oLangRec.GetString(45), frmVideo->GetName(), "cbxSize" );
	cbxSize->SetActiveIndex(m_sysProp.m_videoProp.nResolution ? 1 : 0);  //�������ļ���������ʾ�ڿؼ��� Michael Chen 2005-04-22
	...
	cbxMeleeMode = (CCheckGroup*)frmGameOption->Find("cbxMeleeModel");
	if (!cbxMeleeMode) return Error(g_oLangRec.GetString(45), frmGameOption->GetName(), "cbxMeleeMode");

	cbxIs60Fps = (CCheckGroup*)frmGameOption->Find("cbxIs60Fpsl");
	if (!cbxIs60Fps) return Error(g_oLangRec.GetString(45), frmGameOption->GetName(), "cbxIs60Fpsl");

	cbxRightClick = (CCheckGroup*)frmGameOption->Find("cbxRightClick");
	if (!cbxRightClick) return Error(g_oLangRec.GetString(45), frmGameOption->GetName(), "cbxRightClick");
	
	cbxIsTop1CamMode = (CCheckGroup*)frmGameOption->Find("cbxIsTop1CamMode");
	if (!cbxIsTop1CamMode) return Error(g_oLangRec.GetString(45), frmGameOption->GetName(), "cbxIsTop1CamMode");

	cbxIsFairyShow = (CCheckGroup*)frmGameOption->Find("cbxIsFairyShow");
	if (!cbxIsFairyShow) return Error(g_oLangRec.GetString(45), frmGameOption->GetName(), "cbxIsFairyShow");
	
	cbxIsWingShow = (CCheckGroup*)frmGameOption->Find("cbxIsWingShow");
	if (!cbxIsWingShow) return Error(g_oLangRec.GetString(45), frmGameOption->GetName(), "cbxIsWingShow");
	
	cbxIsOtherShow = (CCheckGroup*)frmGameOption->Find("cbxIsOtherShow");
	if (!cbxIsOtherShow) return Error(g_oLangRec.GetString(45), frmGameOption->GetName(), "cbxIsOtherShow");

	cbxIsItemNameShow = (CCheckGroup*)frmGameOption->Find("cbxIsItemNameShow");
	if (!cbxIsItemNameShow) return Error(g_oLangRec.GetString(45), frmGameOption->GetName(), "cbxIsItemNameShow");
	
	cbxIsChaNameShow = (CCheckGroup*)frmGameOption->Find("cbxIsChaNameShow");
	if (!cbxIsChaNameShow) return Error(g_oLangRec.GetString(45), frmGameOption->GetName(), "cbxIsChaNameShow");
	...
	cbxIsMobNameShow = (CCheckGroup*)frmGameOption->Find("cbxIsMobNameShow");
	if (!cbxIsMobNameShow) return Error(g_oLangRec.GetString(45), frmGameOption->GetName(), "cbxIsMobNameShow");
...
void CSystemMgr::End()
{
	...
	if (cbxSize)
		m_sysProp.m_videoProp.nResolution = cbxSize->GetActiveIndex() == 0 ? false : true;
...
void CSystemMgr::_evtVideoFormMouseEvent(CCompent *pSender, int nMsgType, int x, int y, DWORD dwKey)
{
	...
	{			
		...
		CForm* f = CFormMgr::s_Mgr.Find("frmTargetInfo");
		if (f)
		{
			if (width == 800 || width == 720)
			{
				f->SetPos(270, f->fsTop);	//fix bug with lower resolution 
				f->Refresh();
			}			
			C3DCompent* p3D = dynamic_cast<C3DCompent*>(f->Find("d3dTarget"));
			RECT rt;
			rt.left = p3D->GetX();
			rt.right = p3D->GetX2();
			rt.top = p3D->GetY();
			rt.bottom = p3D->GetY2();
			g_stUIStart.GetMainTarget()->MoveWnd(rt);
		}
	...
	else if (name == "btnNo" ||name=="btnClose" )       
	{
		...
		//g_stUISystem.cbxSize->GetList()->GetItems()->Select(g_nCbxSize);
...
void CSystemMgr::_evtGameOptionFormMouseDown(CCompent *pSender, int nMsgType, int x, int y, DWORD dwKey)
{
	...
	if(pGroup)
	{
		...
		if (g_stUISystem.m_sysProp.m_gameOption.bAppMode != bAppMode)
		{
	...
	pGroup = g_stUISystem.cbxMeleeMode;
	if (pGroup)
	{
		bool bMeleeMode = pGroup->GetActiveIndex() == 1 ? true : false;
		g_stUISystem.m_sysProp.m_gameOption.bMeleeMode = bMeleeMode;

		::WritePrivateProfileString("gameOption", "meleemode", bMeleeMode ? "1" : "0", "./user/system.ini");
	}


	pGroup = g_stUISystem.cbxRightClick;
	
	if (pGroup)
	{
		bool bIsRightClick = pGroup->GetActiveIndex() == 1 ? true : false;
		g_stUISystem.m_sysProp.m_gameOption.bIsRightClick = bIsRightClick;

		::WritePrivateProfileString("gameOption", "R.Click", bIsRightClick ? "1" : "0", "./user/system.ini");
	}
	
	pGroup = g_stUISystem.cbxIsTop1CamMode;
	if(pGroup)
	{
		bool bIsTop1CamMode = pGroup->GetActiveIndex() == 1 ? true : false;
		g_stUISystem.m_sysProp.m_gameOption.bIsTop1CamMode = bIsTop1CamMode;
		::WritePrivateProfileString("gameOption", "camMode", bIsTop1CamMode ? "1" : "0", "./user/system.ini");
	}

	pGroup = g_stUISystem.cbxIsFairyShow;
	if (pGroup)
	{
		bool bIsFairyShow = pGroup->GetActiveIndex() == 1 ? true : false;
		if (g_stUISystem.m_sysProp.m_gameOption.bIsFairyShow != bIsFairyShow)	//way to optimize functions @mothannakh
		{
			g_stUISystem.m_sysProp.m_gameOption.bIsFairyShow = bIsFairyShow;
			CCharacter::SetIsShowPets(bIsFairyShow);
			g_stUIStart.RefreshPet();
			//for each player in scene, re-render them.
			CGameScene* curScene = g_pGameApp->GetCurScene();
			CCharacter* pCha = curScene->_pChaArray;
			...
			::WritePrivateProfileString("effects", "fairy", bIsFairyShow ? "1" : "0", "./user/system.ini");
		}

	}

	pGroup = g_stUISystem.cbxIsWingShow;
	if (pGroup)
	{
		bool bIsWingShow = pGroup->GetActiveIndex() == 1 ? true : false;
		if (g_stUISystem.m_sysProp.m_gameOption.bIsWingShow != bIsWingShow)	//way to optimize functions @mothannakh
		{
			g_stUISystem.m_sysProp.m_gameOption.bIsWingShow = bIsWingShow;
			CCharacter::SetIsShowWings(bIsWingShow);
			g_stUIStart.RefreshPet();
			//for each player in scene, re-render them.
			CGameScene* curScene = g_pGameApp->GetCurScene();
			CCharacter* pCha = curScene->_pChaArray;
			...
			::WritePrivateProfileString("effects", "wing", bIsWingShow ? "1" : "0", "./user/system.ini");
		}
	}

	pGroup = g_stUISystem.cbxIsOtherShow;
	if (pGroup)
	{
		bool bIsOtherShow = pGroup->GetActiveIndex() == 1 ? true : false;
		if (g_stUISystem.m_sysProp.m_gameOption.bIsOtherShow != bIsOtherShow)	//way to optimize functions @mothannakh
		{
			g_stUISystem.m_sysProp.m_gameOption.bIsOtherShow = bIsOtherShow;
			CCharacter::SetIsShowEffects(bIsOtherShow);
			g_stUIStart.RefreshPet();
			//for each player in scene, re-render them.
			CGameScene* curScene = g_pGameApp->GetCurScene();
			CCharacter* pCha = curScene->_pChaArray;
			...
			::WritePrivateProfileString("effects", "other", bIsOtherShow ? "1" : "0", "./user/system.ini");
		}

	}
	
	pGroup = g_stUISystem.cbxIsItemNameShow;
	if (pGroup)
	{
		bool bIsItemNameShow = pGroup->GetActiveIndex() == 1 ? true : false;
		g_stUISystem.m_sysProp.m_gameOption.bIsItemNameShow = bIsItemNameShow;
		//g_pGameApp->SysInfo(bIsItemNameShow ? "Enable Show Item Name" : "Disable Show Item Name");
		::WritePrivateProfileString("gameOption", "itemname", bIsItemNameShow ? "1" : "0", "./user/system.ini");
	}
	
	pGroup = g_stUISystem.cbxIsChaNameShow;
	if (pGroup)
	{
		bool bIsChaNameShow = pGroup->GetActiveIndex() == 1 ? true : false;
		g_stUISystem.m_sysProp.m_gameOption.bIsChaNameShow = bIsChaNameShow;
		//g_pGameApp->SysInfo(bIsChaNameShow ? "Enable Show Cha Name" : "Disable Show Cha Name");
		::WritePrivateProfileString("gameOption", "chaname", bIsChaNameShow ? "1" : "0", "./user/system.ini");
	}
	...
	pGroup = g_stUISystem.cbxIsMobNameShow;
	if (pGroup)
	{
		bool bIsMobNameShow = pGroup->GetActiveIndex() == 1 ? true : false;
		g_stUISystem.m_sysProp.m_gameOption.bIsMobNameShow = bIsMobNameShow;
		//g_pGameApp->SysInfo(bIsMobNameShow ? "Enable Show Mob Name" : "Disable Show Mob Name");
		::WritePrivateProfileString("gameOption", "mobname", bIsMobNameShow ? "1" : "0", "./user/system.ini");
	}

	pGroup = g_stUISystem.cbxIs60Fps;
    if (pGroup)
    {
		char szBuf[256] = { 0 };
		bool bIs60Fps = pGroup->GetActiveIndex() == 1 ? true : false;
		if(bIs60Fps!= g_stUISystem.m_sysProp.m_gameOption.bIs60Fps)
		{			
			g_stUISystem.m_sysProp.m_gameOption.bIs60Fps = bIs60Fps;
			Is60FPSMode = bIs60Fps;

			if (pGroup->GetActiveIndex() == 0)
			{
				sprintf(szBuf, "Please switch character to update framerate.");
				g_stUIBox.ShowMsgBox(nullptr, szBuf);
			}

			::WritePrivateProfileString("gameOption", "frames", bIs60Fps ? "1" : "0", "./user/system.ini");
		}		
...
void CSystemMgr::_evtGameOptionFormBeforeShow(CForm* pForm, bool& IsShow)
{
	...
	pGroup = g_stUISystem.cbxIs60Fps;
	if( pGroup )
		pGroup->SetActiveIndex(g_stUISystem.m_sysProp.m_gameOption.bIs60Fps ? 1 : 0 );

	pGroup = g_stUISystem.cbxRightClick;
	if( pGroup )
		pGroup->SetActiveIndex(g_stUISystem.m_sysProp.m_gameOption.bIsRightClick ? 1 : 0 );

	pGroup = g_stUISystem.cbxMeleeMode;
	if( pGroup )
		pGroup->SetActiveIndex(g_stUISystem.m_sysProp.m_gameOption.bMeleeMode ? 1 : 0 );
	
	pGroup = g_stUISystem.cbxIsTop1CamMode;
	if( pGroup )
		pGroup->SetActiveIndex(g_stUISystem.m_sysProp.m_gameOption.bIsTop1CamMode ? 1 : 0);

	pGroup = g_stUISystem.cbxIsFairyShow;
	if( pGroup )
		pGroup->SetActiveIndex(g_stUISystem.m_sysProp.m_gameOption.bIsFairyShow ? 1 : 0);
	
	pGroup = g_stUISystem.cbxIsWingShow;
	if( pGroup )
		pGroup->SetActiveIndex(g_stUISystem.m_sysProp.m_gameOption.bIsWingShow ? 1 : 0);
	
	pGroup = g_stUISystem.cbxIsOtherShow;
	if( pGroup )
		pGroup->SetActiveIndex(g_stUISystem.m_sysProp.m_gameOption.bIsOtherShow ? 1 : 0);

	pGroup = g_stUISystem.cbxIsItemNameShow;
    if (pGroup)
        pGroup->SetActiveIndex(g_stUISystem.m_sysProp.m_gameOption.bIsItemNameShow ? 1 : 0);
	
	pGroup = g_stUISystem.cbxIsChaNameShow;
    if (pGroup)
        pGroup->SetActiveIndex(g_stUISystem.m_sysProp.m_gameOption.bIsChaNameShow ? 1 : 0);

	pGroup = g_stUISystem.cbxIsMobNameShow;
    if (pGroup)
        pGroup->SetActiveIndex(g_stUISystem.m_sysProp.m_gameOption.bIsMobNameShow ? 1 : 0);
...
void CSystemMgr::_OnDragMenu(CGuiData *pSender, int x, int y, DWORD key)
{
	if (g_stUISystem.frmSystem->GetTop()<=0)
	{
		g_stUISystem.frmSystem->SetPos(g_stUISystem.frmSystem->GetLeft(), 0);
		g_stUISystem.frmSystem->Refresh();
	}

	if (g_stUISystem.frmSystem->GetLeft()<=0)
	{
		g_stUISystem.frmSystem->SetPos(0, g_stUISystem.frmSystem->GetTop());
		g_stUISystem.frmSystem->Refresh();
	}

	if (g_stUISystem.frmSystem->GetBottom()>=GetRender().GetScreenHeight())
	{
		g_stUISystem.frmSystem->SetPos( g_stUISystem.frmSystem->GetLeft(),  GetRender().GetScreenHeight() - g_stUISystem.frmSystem->GetHeight() );
		g_stUISystem.frmSystem->Refresh();
	}

	if (g_stUISystem.frmSystem->GetRight()>=GetRender().GetScreenWidth())
	{
		g_stUISystem.frmSystem->SetPos(GetRender().GetScreenWidth() - g_stUISystem.frmSystem->GetWidth(), g_stUISystem.frmSystem->GetTop());
		g_stUISystem.frmSystem->Refresh();
	}
}
