...
bool CMisLogForm::Init()
{
	...
	m_pForm->evtMouseDragEnd = _OnDragMission;
...
void CMisLogForm::_OnDragMission(CGuiData *pSender, int x, int y, DWORD key)
{
	if (g_stUIMisLog.m_pForm->GetTop()<=0)
	{
		g_stUIMisLog.m_pForm->SetPos(g_stUIMisLog.m_pForm->GetLeft(), 0);
		g_stUIMisLog.m_pForm->Refresh();
	}

	if (g_stUIMisLog.m_pForm->GetLeft()<=0)
	{
		g_stUIMisLog.m_pForm->SetPos(0, g_stUIMisLog.m_pForm->GetTop());
		g_stUIMisLog.m_pForm->Refresh();
	}

	if (g_stUIMisLog.m_pForm->GetBottom()>=GetRender().GetScreenHeight())
	{
		g_stUIMisLog.m_pForm->SetPos( g_stUIMisLog.m_pForm->GetLeft(),  GetRender().GetScreenHeight() - g_stUIMisLog.m_pForm->GetHeight() );
		g_stUIMisLog.m_pForm->Refresh();
	}

	if (g_stUIMisLog.m_pForm->GetRight()>=GetRender().GetScreenWidth())
	{
		g_stUIMisLog.m_pForm->SetPos(GetRender().GetScreenWidth() - g_stUIMisLog.m_pForm->GetWidth(), g_stUIMisLog.m_pForm->GetTop());
		g_stUIMisLog.m_pForm->Refresh();
	}
}

void CMisLogForm::_ItemClickEvent( std::string strItem )
{
