...
bool CMouseDown::ActAttackCha( CCharacter* pCha, CSkillRecord* pSkill, CCharacter* pTarget, bool isTrace, bool IsForce, bool IsCompart )
{
	...
	//@mothannakh
	if( !pTarget->IsNPC() && pCha != pTarget && pTarget->getChaCtrlType() != enumCHACTRL_Mount && !g_stUIMap.IsPKSilver() ){
...
bool CMouseDown::ActAttackArea( CCharacter* pCha, CSkillRecord* pSkill, int nScrX, int nScrY, bool IsForce, bool IsCompart )
{
    ...
    attack->SetSkill( pSkill );
	attack->SetAttackPoint( nScrX, nScrY );
	const auto cmd = CCommandObj::GetReadyCommand();
	const auto item = dynamic_cast<CItemCommand*>(cmd);
	if (!item || item->GetTotalNum() > 1) {
		attack->SetCommand(cmd);
	}
	else {
		attack->SetCommand(nullptr);
	}
...
bool CMouseDown::ActDummyObj( CCharacter* pCha, int x, int y, int h, int t_angle, int action )
{
	...
	if(pCha->GetIsOnMount()) return false; 		// return false for seat on objects @mothannakh
...
void CMouseDown::SetAltFollow(CCharacter* Pmain, CCharacter* Target)
{
	_pAutoAttack->Follow(Pmain, Target);
}