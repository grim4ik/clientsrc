...
CForm*	CUIGuildMgr::m_pGuildColForm=nullptr;
...
CLabelEx*	CUIGuildMgr::m_plabGuildMoney=nullptr;
CLabelEx*	CUIGuildMgr::m_plabGuildExperience=nullptr;
...
CTextButton*	CUIGuildMgr::m_pbtnGuildDismiss=nullptr;
...
CTextButton*	CUIGuildMgr::m_pbtnHouse=nullptr;
CTextButton*	CUIGuildMgr::m_pbtnColour=nullptr;
...
/*CTextButton*	CUIGuildMgr::m_pbtnattrPR=nullptr;
CTextButton*	CUIGuildMgr::m_pbtnattrMS=nullptr;
CTextButton*	CUIGuildMgr::m_pbtnattrASPD=nullptr;
CTextButton*	CUIGuildMgr::m_pbtnattrMX=nullptr;
CTextButton*	CUIGuildMgr::m_pbtnattrDEF=nullptr;
CTextButton*	CUIGuildMgr::m_pbtnattrHIT=nullptr;
CTextButton*	CUIGuildMgr::m_pbtnattrFLEE=nullptr;
CTextButton*	CUIGuildMgr::m_pbtnattrHPR=nullptr;
CTextButton*	CUIGuildMgr::m_pbtnattrSPR=nullptr;
CTextButton*	CUIGuildMgr::m_pbtnattrHP=nullptr;
CTextButton*	CUIGuildMgr::m_pbtnattrSP=nullptr;*/

CEdit* CUIGuildMgr::m_edtRed=nullptr;
CEdit* CUIGuildMgr::m_edtGreen=nullptr;
CEdit* CUIGuildMgr::m_edtBlue=nullptr;
CTextButton*	CUIGuildMgr::m_btnConfCol=nullptr;
CTextButton*	CUIGuildMgr::m_btnNext=nullptr;
CTextButton*	CUIGuildMgr::m_btnPrev=nullptr;
CTextButton*	CUIGuildMgr::m_btnDisCol=nullptr;
CImage*	CUIGuildMgr::m_imgPreview=nullptr;
CLabel*	CUIGuildMgr::m_gCircleCount=nullptr;

CTreeView*	CUIGuildMgr::m_trvPerm=nullptr;
std::string	CUIGuildMgr::attrName="";
...
bool CUIGuildMgr::Init()
{
	...
	m_pGuildMgrForm->evtMouseDragEnd=_OnDragGuild;
...
void CUIGuildMgr::_OnClickConfirmPerm(CGuiData *pSender, int x, int y, DWORD key){
	...
	if ( pSelfData && perm==emGldPermMgr){
		...
		if (!pRow){
			CBoxMgr::ShowMsgBox( nullptr, g_oLangRec.GetString(600), true );
			return;
		}
...
/*bool CUIGuildMgr::RefreshMobModelPos(int index){	
	...
	|| (g_stUISystem.m_sysProp.m_videoProp.bPixel1024 && (m_pGuildMgrForm ->GetLeft()+170+32 > 1024 ||  m_pGuildMgrForm->GetTop()+100+index*87+32> 768))
	|| (!g_stUISystem.m_sysProp.m_videoProp.bPixel1024 && (m_pGuildMgrForm ->GetLeft()+170+32 > 800 ||  m_pGuildMgrForm->GetTop()+100+index*87+32 > 600))
	){
		return false;
	}
...
void CUIGuildMgr::_OnClickEditMottoName(CGuiData *pSender, int x, int y, DWORD key)
{
	...
	{
		CBoxMgr::ShowMsgBox( nullptr, g_oLangRec.GetString(594), true );
	}
}
...
void CUIGuildMgr::_OnClickRecruit(CGuiData *pSender, int x, int y, DWORD key)
{
	...
	if ( pSelfData && perm==emGldPermRecruit)
	{
		...
		if (!pRow)
		{
			CBoxMgr::ShowMsgBox( nullptr, g_oLangRec.GetString(597), true );
			return;
		}
	...
	{
		CBoxMgr::ShowMsgBox( nullptr, g_oLangRec.GetString(598), true );
	}
}
...
void CUIGuildMgr::_OnClickRefuse(CGuiData *pSender, int x, int y, DWORD key)
{
	...
	{
		CBoxMgr::ShowMsgBox( nullptr, g_oLangRec.GetString(599), true );
	}
}
...
void CUIGuildMgr::_OnClickKick(CGuiData *pSender, int x, int y, DWORD key)
{
	...
	if ( pSelfData && perm==emGldPermKick)
	{
		...
		CGuildMemberData* pMemberData=static_cast<CGuildMemberData*>(pRow->GetPointer());
		std::string str=g_oLangRec.GetString(601)+pMemberData->GetName()+g_oLangRec.GetString(602);
		CBoxMgr::ShowSelectBox(_OnPassKick,str.c_str(),true);
	}
	else
	{
		CBoxMgr::ShowMsgBox( nullptr, g_oLangRec.GetString(603), true );
	}
}
...
void CUIGuildMgr::_EnterGuildHouse(CGuiData *pSender, int x, int y, DWORD key){
	CS_BeginAction(g_stUIBoat.GetHuman(), enumACTION_ENTERGUILDHOUSE, nullptr); 
}

void CUIGuildMgr::_OnClickMottoFormOK(CGuiData *pSender, int x, int y, DWORD key)
{
	std::string name=m_pedtGuildMottoName->GetCaption();
...
void CUIGuildMgr::_OnPassDismiss(CCompent *pSender, int nMsgType, int x, int y, DWORD dwKey)
{
	...
	if( !pBox ) return;
	std::string str=pBox->edtPassword->GetCaption();
	if (str.length()>0)
	{
...
void CUIGuildMgr::_OnPassKick(CCompent *pSender, int nMsgType, int x, int y, DWORD dwKey)
{
	...
	if (!pRow)
	{
		CBoxMgr::ShowMsgBox( nullptr, g_oLangRec.GetString(614), true );
		return;
	}
	CGuildMemberData* pMemberData=static_cast<CGuildMemberData*>(pRow->GetPointer());
...
void CUIGuildMgr::_OnDragGuild(CGuiData *pSender, int x, int y, DWORD key)
{
	if (g_stUIGuildMgr.m_pGuildMgrForm->GetTop()<=0)
	{
		g_stUIGuildMgr.m_pGuildMgrForm->SetPos(g_stUIGuildMgr.m_pGuildMgrForm->GetLeft(), 0);
		g_stUIGuildMgr.m_pGuildMgrForm->Refresh();
	}

	if (g_stUIGuildMgr.m_pGuildMgrForm->GetLeft()<=0)
	{
		g_stUIGuildMgr.m_pGuildMgrForm->SetPos(0, g_stUIGuildMgr.m_pGuildMgrForm->GetTop());
		g_stUIGuildMgr.m_pGuildMgrForm->Refresh();
	}

	if (g_stUIGuildMgr.m_pGuildMgrForm->GetBottom()>=GetRender().GetScreenHeight())
	{
		g_stUIGuildMgr.m_pGuildMgrForm->SetPos( g_stUIGuildMgr.m_pGuildMgrForm->GetLeft(),  GetRender().GetScreenHeight() - g_stUIGuildMgr.m_pGuildMgrForm->GetHeight() );
		g_stUIGuildMgr.m_pGuildMgrForm->Refresh();
	}

	if (g_stUIGuildMgr.m_pGuildMgrForm->GetRight()>=GetRender().GetScreenWidth())
	{
		g_stUIGuildMgr.m_pGuildMgrForm->SetPos(GetRender().GetScreenWidth() - g_stUIGuildMgr.m_pGuildMgrForm->GetWidth(), g_stUIGuildMgr.m_pGuildMgrForm->GetTop());
		g_stUIGuildMgr.m_pGuildMgrForm->Refresh();
	}
}
