...
{
	if( !CGameApp::GetCurScene() ) return nullptr;
...
void	NetEndPlay( char iCharNum, NetChaBehave chabehave[] )
{
    ...
    if( !pScene ) 
    {
        ...
        {
            LG( "protocol", "msgNetEndPlay CreateScene return nullptr\n" );
            return;
        }
    }
...
void NetActorSkillRep( unsigned int nID, stNetNotiSkillRepresent &SSkillRep)
{
	...
	else
	{
        ...
        if( !g_state )
        {
			...
			if( !p ) 
			{
				LG( "protocol", "NetActorSkillRep GetSkillRecordInfo(%d) return nullptr\n", SSkillRep.lSkillID );
				return;
			}
...
void NetActorSkillEff(unsigned int nID, stNetNotiSkillEffect &SkillEff)
{
	...
	if( !pTarget ) 
	{
		LG( "protocol", "msgNetActorSkillEff Error, Target[%u] is nullptr, SkillID[%u]\n", nID, SkillEff.lSkillID );
		return;
	}
	...
	if( lean.chState==0 )
	{
		...
		else
		{
			...
			//if( !cha->GetActor()->GetCurState() )
			{
                ...
                {
                    g_pGameApp->AddTipText( "NetActorLean NewState return nullptr\n" );
                    return;
                }
			}
		}
...
void NetSwitchMap(stNetSwitchMap &switchmap)
{
	...
	// in-game shop crash fix when cha teleports with shop opened (6/28/2023)
	...
	if( !pInfo ) 
	{
		LG( "switchmap", "msgNetSwitchMap - GetMapInfo(%s) return nullptr\n", switchmap.szMapName );
		return;
	}
	...
	if( !s )	//����Ϸ����
	{
        ...
        if( !s ) 
        {
            LG( "switchmap", "msgNetSwitchMap(%s) CreateScene return nullptr\n", switchmap.szMapName );
            return;
        }
...
{    
    ...
    pItem = pScene->AddSceneItem(info.lID, 0, info.isProtected ? true : false);
    if(pItem==nullptr)
    {
	...
	pItem->SetIsProtected(info.isProtected ? true : false);
	pItem->SetTicks(info.startTick, info.curTick, info.endProtTick);
