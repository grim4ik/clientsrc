...
CTreeGridNode* CChat::_pGMNode = nullptr;
...
bool CChat::Init()
{
	...
	_frmQQ->evtMouseDragEnd= _OnDragQQ;
...
void CChat::_OnMouseFrndMenu(CGuiData *pSender, int x, int y, DWORD key)
{
	...
	{
		std::string str=g_oLangRec.GetString(480);
...
void CChat::_OnMouseMasterMenu(CGuiData *pSender, int x, int y, DWORD key)
{
	...
	else if(strCommand == g_oLangRec.GetString(854))	// �����ϵ��ͽ�ܽ��ʦ����
	{
		...
		if( pMain && pMain->getGameAttr() )
		{
			...
			char szBuffer[256] = {0};
			sprintf(szBuffer, g_oLangRec.GetString(889),_curSelectMember->GetName(),0); // sprintf(szBuffer, g_oLangRec.GetString(889), _curSelectMember->GetName(), nLevel * 500); 
...
void CChat::_OnMouseStudentMenu(CGuiData *pSender, int x, int y, DWORD key)
{
	...
	else if(strCommand == g_oLangRec.GetString(854))	// �����ϵ��ʦ�����ͽ�ܣ�
	{
		...
		if( pMain && pMain->getGameAttr() )
		{
			...
			char szBuffer[256] = {0};
			sprintf(szBuffer, g_oLangRec.GetString(856), _curSelectMember->GetName(), 0); //sprintf(szBuffer, g_oLangRec.GetString(856), _curSelectMember->GetName(),nLevel * 5);
...
{
	std::string name=_pEditFrndName->GetCaption();
	if (!name.empty())
	{
...
{
	std::string name=_pEditBlockName->GetCaption();
	if (!name.empty())
	{
...
void CChat::_OnDragEnd(CForm* pTargetForm, CTreeView *pTree, CTreeNodeObj* pNode, CItemObj* pItem, int x, int y, DWORD key  )
{
	...
	if (!pSessForm) return;
	std::string strCaption=pNode->GetCaption();
...
void CChat::_OnDragQQ(CGuiData *pSender, int x, int y, DWORD key)
{
	if (g_stUIChat._frmQQ->GetTop()<=0)
	{
		g_stUIChat._frmQQ->SetPos(g_stUIChat._frmQQ->GetLeft(), 0);
		g_stUIChat._frmQQ->Refresh();
	}

	if (g_stUIChat._frmQQ->GetLeft()<=0)
	{
		g_stUIChat._frmQQ->SetPos(0, g_stUIChat._frmQQ->GetTop());
		g_stUIChat._frmQQ->Refresh();
	}

	if (g_stUIChat._frmQQ->GetBottom()>=GetRender().GetScreenHeight())
	{
		g_stUIChat._frmQQ->SetPos( g_stUIChat._frmQQ->GetLeft(),  GetRender().GetScreenHeight() - g_stUIChat._frmQQ->GetHeight() );
		g_stUIChat._frmQQ->Refresh();
	}

	if (g_stUIChat._frmQQ->GetRight()>=GetRender().GetScreenWidth())
	{
		g_stUIChat._frmQQ->SetPos(GetRender().GetScreenWidth() - g_stUIChat._frmQQ->GetWidth(), g_stUIChat._frmQQ->GetTop());
		g_stUIChat._frmQQ->Refresh();
	}
}
