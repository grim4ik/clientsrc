...
int CGameApp::Run()
{
	...
	while( _isRun ) 	
	{
		...
		if( _pCurScene )
		{
			...
			g_NetIF->PeekPacket(1);	// �����ͻ���CPUռ����100%
			if( _pSteady->Run() )
			{
...
skip:
			...
			if( int(xp / SHOWRSIZE) >= (destxp /SHOWRSIZE))
			{
				...
				{
					MessageBox(nullptr, g_oLangRec.GetString(74),"INFO",0);
					xp = xp1;
					yp = yp1;
...
CGameScene*	CGameApp::CreateScene( stSceneInitParam* param )
{
	...
	if( param->nTypeID>=enumSceneEnd ) return nullptr;
...
void CGameApp::MsgBoxTime(const char* szTitle, int iSeconds) {
	g_stUIBox.ShowMsgTime(nullptr, szTitle, iSeconds);
}
	...
	else
	{
		...
		for (i = 0; i < MAX_CQUEUE; i++)
		{
			...
			if (GetCurTick() < tmp->dwBeginTime + TIME_CQUEUE)
			{
				...
				g_pGameApp->GetFont()->DrawText(tmp->szText, x+5, y+10, (DWORD)dwColor);
...
void CGameApp::ShowStateHint( int x, int y, CChaStateMgr::stChaState stateData){
	...
	{
		//star //  a lame way to load hardcoded description and names @moth-- just add cases for other states here and ^ above 
		switch (stateData.pInfo->chID)
		{
		case 48: {
			if (stateData.chStateLv == 3)
	...
	if(stateData.lTimeRemaining > 0){
		...
		//_BottomFont.DrawTextShadow(szTime, x+1, y+1, x, y, D3DXCOLOR(0,0,0,1), (DWORD)0xff808000 );
...
void CGameApp::RenderStateHint(int x, int y, CChaStateMgr::stChaState stateData) {
	if (stateData.lTimeRemaining > 0) {
		//background icon
		CGuiPic timericon;
		timericon.LoadImage("texture/ui/custom/colourpreview.png", 20, 13, 0, 0, 0, 1, 1);
		timericon.Render(x + 1, y + 21);
		//background end
		char szTime[32];
		int minutes = stateData.lTimeRemaining / 60;
		int seconds = stateData.lTimeRemaining % 60;
		...
		CGuiFont::s_Font.Render(1, szTime, x + 3, y + 21, COLOR_BLACK, scale);
	}
...
{
	const int PROGRESS_CRYCLE  = 1000;			// һ����ѭ�����룩
	const int PROGRESS_WIDTH   = 523;
	const int PROGRESS_X       = 132;
	const int PROGRESS_Y       = 447;
	...
	if(frmLoading)
	{
		...
		if(imgLoading)
		{
			float fPrecent = ((GetCurTick() - _dwLoadingTick) % PROGRESS_CRYCLE) / (float)(PROGRESS_CRYCLE);	// �ڵ�ǰ�������İٷֱ�
		...
		if( labLoad )
		{
			...
			int y = ( GetWindowHeight() == 800 ) ? 523 : 679;
			labLoad->SetPos( x, y );
		}
	}
}
...
void CGameApp::InitAllTable()
{
	...
	const auto pEffSet = new CEffectSet(0, g_Config.m_nMaxEffectType);
	...
	const auto pSkillSet = new CSkillRecordSet(0, 500);
	pSkillSet->LoadRawDataInfo("scripts/table/skillinfo", bBinary, encode);

	const auto pMapSet = new CMapSet(0, 100);
	pMapSet->LoadRawDataInfo("scripts/table/mapinfo", bBinary, encode);

	const auto pChaSetAttrib = new CChaRecordSet(0, 2500);
	pChaSetAttrib->LoadRawDataInfo("scripts/table/Characterinfo", bBinary, encode);

	const auto pItemSet = new CItemRecordSet(0, g_Config.m_nMaxItemType);
	pItemSet->LoadRawDataInfo("scripts/table/iteminfo", bBinary, encode);

    CEventRecordSet* pEvent = new CEventRecordSet( 0,10 );
    pEvent->LoadRawDataInfo("scripts/table/objevent", bBinary);

	const auto pArea = new CAreaSet(0, 300);
	pArea->LoadRawDataInfo("scripts/table/AreaSet", bBinary, encode);
    ...
    pState->LoadRawDataInfo("scripts/table/skilleff", bBinary, encode);
	...
	const auto pItemRefineSet = new CItemRefineSet(0, g_Config.m_nMaxItemType);
	pItemRefineSet->LoadRawDataInfo("scripts/table/ItemRefineInfo", bBinary, encode);

	const auto pItemRefineEffectSet = new CItemRefineEffectSet(0, 5000);
	pItemRefineEffectSet->LoadRawDataInfo("scripts/table/ItemRefineEffectInfo", bBinary, encode);
	...
	const auto MountHelperSet = new MountHelper(0, 300);
	MountHelperSet->LoadRawDataInfo("scripts/table/MountInfo", bBinary, encode);
}
