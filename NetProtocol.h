...
{
	long	lWorldID;
	long	lHandle;		// Server information, the original value is returned 
	long	lEntityAddr;	// The physical address of the server 
	long	lID;
	Point	SPos;
	short	sAngle;
	short	sNum;			// Number of items
	char	chAppeType;		// Type of birth (see CompCommand.h EItemAppearType��
	long	lFromID;		// The ID of the throwing source, if it is 0, there is no throwing source 
	long	isProtected; // flag which signifies if item is protected, 0 - not protected, 1 - protected
	long 	startTick;
	long	curTick;
	long	endProtTick;
	stNetEvent	SEvent;
