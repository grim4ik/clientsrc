...
bool CEquipMgr::Init()
{
	...
	frmSkill->evtMouseDragEnd = _OnDragSkill;
	...
	frmInv->evtMouseDragEnd = _OnDragInventory;
	...
	//
	frmDailyRewards = _FindForm("frmDailyRewards");
	if (!frmDailyRewards) return false;
	imgfrmDailyRewards= dynamic_cast<CImage*>(frmDailyRewards->Find("imgfrmDailyRewards"));
	if (!imgfrmDailyRewards)return Error(g_oLangRec.GetString(45), frmDailyRewards->GetName(), "imgfrmDailyRewards");

	CTextButton* btnClose = dynamic_cast<CTextButton*>(frmDailyRewards->Find("btnClose"));
	if (!btnClose) return Error(g_oLangRec.GetString(45), frmDailyRewards->GetName(), "btnClose");
	btnClose->evtMouseClick = _evtButtonClickEvent;

	CTextButton* btnReward = dynamic_cast<CTextButton*>(frmDailyRewards->Find("btnReward"));
	if (!btnReward) return Error(g_oLangRec.GetString(45), frmDailyRewards->GetName(), "btnReward");
	btnReward->evtMouseClick = _ClickDaily;
	//
	...
	*/
	...
	int nIndex = 0;
	for (size_t i = 1; i < MAX_FAST_ROW-1; i++) {
		for (size_t j = 0; j < MAX_FAST_COL; j++) {
			if (_pFastCommands[j]) {
	...
	CTextButton* btnOpenCards = (CTextButton*)frmInv->Find("openCards");
	btnOpenCards->evtMouseClick = _ClickOpenCards;

	
	return true;
}

void CEquipMgr::_ClickDaily(CGuiData* pSender, int x, int y, DWORD key)
{
	g_stUIEquip.DailyReward();
}

void CEquipMgr::_ClickOpenCards(CGuiData *pSender, int x, int y, DWORD key ){
	CForm* frmCards = _FindForm("frmCards");
	frmCards->SetIsShow(!frmCards->GetIsShow());
}

void CEquipMgr::DailyReward()
{
	 for (int n = 1; n <= 31; n++)
	{
		 char serverday[15];
		 sprintf(serverday, "serverday%d", n);
		// g_pGameApp->SysInfo(serverday);
		 ADDReward(serverday);
	}
	

}
void CEquipMgr::ADDReward(CHAR*name)
{

	COneCommand* ItemIcons = dynamic_cast<COneCommand*>(frmDailyRewards->Find(name));	//item rewards  
	char serverday[32];
	sprintf(serverday, "imgClaimed%s", name);

	CCompent* imgrewards = dynamic_cast<CCompent*>(frmDailyRewards->Find(serverday));	//reward Claimed true 
	sprintf(serverday, "imgBlocked%s", name);
	CCompent* imgBlocked = dynamic_cast<CCompent*>(frmDailyRewards->Find(serverday));	//reward locked image 
	sprintf(serverday, "imgLocked%s", name);
	CCompent* imgLocked = dynamic_cast<CCompent*>(frmDailyRewards->Find(serverday));	//blocked reward icon 
	
	//place items inside form
	for (int n = 1; n <= 31; n++)
	{
		CItemRecord* pInfo = GetItemRecordInfo(n);
		sprintf(serverday, "serverday%d", n);
		if (pInfo && 0 == stricmp(name, serverday))
		{
			CItemCommand* pItem = new CItemCommand(pInfo);
			ItemIcons->SetIsDrag(false);
			ItemIcons->AddCommand(pItem);
			imgrewards->SetIsShow(true);
			imgBlocked->SetIsShow(false);
			imgLocked->SetIsShow(false);
		}
		else {

			//hide default icons 
			imgrewards->SetIsShow(false);
			imgBlocked->SetIsShow(true);
			imgLocked->SetIsShow(true);
		}
	}
	//fill items end 
...
void CEquipMgr::SynSkillBag(DWORD dwCharID, stNetSkillBag* pSSkillBag)
{
	...
	{
	case enumSYN_SKILLBAG_INIT: //Initialize the skill bar (send all skill information)
	{
		if (!isloaded)	//@mothannakh 
		{
			lstSailSkill->Clear();
			lstFightSkill->Clear();
			lstLifeSkill->Clear();
	...
	break;
	case enumSYN_SKILLBAG_ADD:  // Add skills (only send information about newly added skills）
	{
		CSkillRecord* pInfo = nullptr;
		CSkillCommand* tmp = nullptr;
		for (int i = 0; i < nCount; i++)
	...
	break;
	case enumSYN_SKILLBAG_MODI: // Modify skills
	{
		CSkillRecord* pInfo = nullptr;
		CSkillCommand* tmp = nullptr;
		for (int i = 0; i < nCount; i++)
...
void CEquipMgr::_evtButtonClickEvent( CGuiData *pSender, int x, int y, DWORD key)
{
	...
	{
		CForm* f = CFormMgr::s_Mgr.Find("frmDailyRewards");
		if (f)
		{
			f->SetIsShow(false);
		}
...
bool CEquipMgr::ExecFastKey(int key) {
	...
	bool ChatBoxActive = true;
	//disable using skills if chat box active
	...
	if (form)
	{
		...
		else ChatBoxActive = false;
	}
	//
	bool FkeysInUse = false;	//to make sure we are using f keys not number keys @mothannakh
	if (key >= 112 && key <= 123) // 112 = f1 to 123 = f12 number keys starts from 49 -
		FkeysInUse = true;
	//key !=8 this one for backspace 	
	if (IsNumberTopBar && !FkeysInUse && !g_pGameApp->IsCtrlPress() && key != 8 && !ChatBoxActive) //top bar using as numbers @mothannakh with ogge help
	{
		WORD NumberID = key - '0';
	...
	if (g_pGameApp->IsShiftPress() && !IsNumberTopBar) {
		key = 2 * MAX_FAST_COL + key;
	}
	else if (IsNumberTopBar && !FkeysInUse)	//check if using top  bar as numbers @mothannakh
	{
		key = 2 * MAX_FAST_COL + key;
	}
	else {
...
void CEquipMgr::_SendThrowData( const stThrow& sthrow, int nThrowNum )
{
	...
	{
		stSelectBox* pSelectBox = g_stUIBox.ShowSelectBox(_evtThrowDeleteConfirm, "Throw away?", true);
...
void CEquipMgr::_evtSpyFormShow(CGuiData *pSender){
	...
	g_stUIEquip.spyModel->setChaCtrlType(0);
...
void CEquipMgr::_evtEquipFormShow(CGuiData *pSender){
	...
	g_stUIEquip.chaModel->setChaCtrlType(0);
...
void CEquipMgr::_evtRMouseGridEvent(CGuiData *pSender,CCommandObj* pItem,int nGridID)
{
	...
	CItemCommand* pItemCommand = dynamic_cast<CItemCommand*>(pItem);
	if( !pItemCommand || pItemCommand->GetItemInfo()->sType!=43 && g_stUISystem.m_sysProp.m_gameOption.bIsRightClick )
	{
...
void CEquipMgr::_OnDragSkill(CGuiData *pSender, int x, int y, DWORD key)
{
	if (g_stUIEquip.frmSkill->GetTop()<=0)
	{
		g_stUIEquip.frmSkill->SetPos(g_stUIEquip.frmSkill->GetLeft(), 0);
		g_stUIEquip.frmSkill->Refresh();
	}

	if (g_stUIEquip.frmSkill->GetLeft()<=0)
	{
		g_stUIEquip.frmSkill->SetPos(0, g_stUIEquip.frmSkill->GetTop());
		g_stUIEquip.frmSkill->Refresh();
	}

	if (g_stUIEquip.frmSkill->GetBottom()>=GetRender().GetScreenHeight())
	{
		g_stUIEquip.frmSkill->SetPos( g_stUIEquip.frmSkill->GetLeft(),  GetRender().GetScreenHeight() - g_stUIEquip.frmSkill->GetHeight() );
		g_stUIEquip.frmSkill->Refresh();
	}

	if (g_stUIEquip.frmSkill->GetRight()>=GetRender().GetScreenWidth())
	{
		g_stUIEquip.frmSkill->SetPos(GetRender().GetScreenWidth() - g_stUIEquip.frmSkill->GetWidth(), g_stUIEquip.frmSkill->GetTop());
		g_stUIEquip.frmSkill->Refresh();
	}
}

void CEquipMgr::_OnDragInventory(CGuiData *pSender, int x, int y, DWORD key)
{
	if (g_stUIEquip.frmInv->GetTop()<=0)
	{
		g_stUIEquip.frmInv->SetPos(g_stUIEquip.frmInv->GetLeft(), 0);
		g_stUIEquip.frmInv->Refresh();
	}

	if (g_stUIEquip.frmInv->GetLeft()<=0)
	{
		g_stUIEquip.frmInv->SetPos(0, g_stUIEquip.frmInv->GetTop());
		g_stUIEquip.frmInv->Refresh();
	}

	if (g_stUIEquip.frmInv->GetBottom()>=GetRender().GetScreenHeight())
	{
		g_stUIEquip.frmInv->SetPos( g_stUIEquip.frmInv->GetLeft(),  GetRender().GetScreenHeight() - g_stUIEquip.frmInv->GetHeight() );
		g_stUIEquip.frmInv->Refresh();
	}

	if (g_stUIEquip.frmInv->GetRight()>=GetRender().GetScreenWidth())
	{
		g_stUIEquip.frmInv->SetPos(GetRender().GetScreenWidth() - g_stUIEquip.frmInv->GetWidth(), g_stUIEquip.frmInv->GetTop());
		g_stUIEquip.frmInv->Refresh();
	}
}
