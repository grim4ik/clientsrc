...
: _pItem(nullptr), _isSelect(false), _nIndex(-1), _isEnabled(true)
, _pList(nullptr)
{
	_pImage = new CGuiPic;
}
...
, _dwFirstShowRow(0), _pList(nullptr), _dwLastShowRow(0), _nShowCount(1000), _nMaxRow(0), _IsMouseFollow(false), _pItemPic(nullptr)
	...
	{	
		if( _IsMouseFollow || (key & Mouse_LDown) || (key & Mouse_LDB) || (key &Mouse_RDown) )
		{
			...
			if( key & Mouse_LDown) 
			{
				...
				// activate mousedown click handler on row item
				if (_items[row]->evtListMouseDown && stricmp ("frmMobInfoDrop" , _pList->GetForm()->GetName() ) == 0  ) {
				
				 _items[row]->evtListMouseDown(_pList, x, y, key, _items[row]);	
...
, evtSelectChange(nullptr), evtListMouseDown(nullptr), evtListMouseDB(nullptr)
{
	_IsFocus = false;
