...
//#endif
	...
	//if(w < 0 && h < 0)
	{
		...
		else if(g_stUISystem.m_sysProp.m_videoProp.nResolution)
		{
			w = LARGE_RES_X;
			h = 768;
		}
    ...
    if(g_stUISystem.m_sysProp.m_videoProp.bFullScreen) // αȫ��
    {
	...
	if(g_stUISystem.m_sysProp.m_videoProp.bFullScreen)
	{
		load->m_hLoading =  CreateWindowEx(WS_EX_TOPMOST, "Loading", "Loading", WS_POPUP /*& ~WS_TILED*/  , 
			0, 0 ,  nSizeW, nSizeH, nullptr, nullptr, g_hInstance, nullptr);
	}
	else
	{
		load->m_hLoading =  CreateWindowEx(WS_EX_TOPMOST, "Loading", "Loading", WS_POPUP /*& ~WS_TILED*/  , 
		//load->m_hLoading =  CreateWindow("Loading", "Loading", WS_POPUP /*& ~WS_TILED*/  , 
			(nSizeW - nWindowWidth) / 2.0, (nSizeH - nWindowHeight) / 2.0 + nCaptionSize + nFrameSize - 1, nWindowWidth, nWindowHeight, nullptr, nullptr, g_hInstance, nullptr);
	}
...
GameLoading* GameLoading::Init(std::string param)
{
...
GameLoading::GameLoading(std::string param)
{
	m_bShow = true;
	m_bWait = true;
