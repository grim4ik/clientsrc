...
CCharacter2D* CNpcTalkMgr::pNpc = nullptr;
C3DCompent* CNpcTalkMgr::p3D = nullptr;
...
bool CNpcTalkMgr::Init()
{
	...
	frmNPCchat->evtMouseDragEnd = _OnDragDialogue;
...
bool CNpcTalkMgr::RefreshNpcModelPos(){	
	...
	|| (g_stUISystem.m_sysProp.m_videoProp.nResolution && (p3D->GetX2() > LARGE_RES_X ||  p3D->GetY2() > 768))
	|| (!g_stUISystem.m_sysProp.m_videoProp.nResolution && (p3D->GetX2() > SMALL_RES_X ||  p3D->GetY2() > 600))
	){
		return false;
	}
...
void CNpcTalkMgr::ShowFuncPage( BYTE byFuncPage , BYTE byCount,BYTE byMisNum, const NET_FUNCPAGE& FuncArray, DWORD dwNpcID ) 
{
	...
	if ( byMisNum >0 )
	{
		memCtrl->SetIsHaveMission(true);		
		memCtrl->SetMissionRowNum( byMisNum);
		for (int i =0 ; i<byMisNum ; i++)
		{
			...
			memCtrl->AddMissionRowContent( i , FuncArray.MisItem[i].szMis);
		}			
	}
	...
	if( nArea){
		...
		sprintf(buf,"texture/ui/custom/npcBackdrop/%d.tga",0);
	}
	...
	if(imgNpcBackDrop){
		...
		if(GetFileAttributes(buf) == INVALID_FILE_ATTRIBUTES){
			imgNpcBackDrop->GetImage()->LoadImage("texture/ui/custom/npcBackdrop/0.tga",55, 44, 0, 0, 0);
		}else{
...
void CNpcTalkMgr::_evtMemSelectChange(CGuiData *pSender)
{
	...
	if (!memo->GetIsHaveItem() &&!memo->GetIsHaveMission() )
		return;

	int nMis= -1;
	int nItem = -1;

	nMis = memo->GetSelectMission();
...
void CNpcTalkMgr::_OnDragDialogue(CGuiData *pSender, int x, int y, DWORD key)
{
	...
	if (g_stUINpcTalk.frmNPCchat->GetTop()<=0)
	{
		g_stUINpcTalk.frmNPCchat->SetPos(g_stUINpcTalk.frmNPCchat->GetLeft(), 0);
		g_stUINpcTalk.frmNPCchat->Refresh();
	}

	if (g_stUINpcTalk.frmNPCchat->GetLeft()<=0)
	{
		g_stUINpcTalk.frmNPCchat->SetPos(0, g_stUINpcTalk.frmNPCchat->GetTop());
		g_stUINpcTalk.frmNPCchat->Refresh();
	}

	if (g_stUINpcTalk.frmNPCchat->GetBottom()>=GetRender().GetScreenHeight())
	{
		g_stUINpcTalk.frmNPCchat->SetPos( g_stUINpcTalk.frmNPCchat->GetLeft(),  GetRender().GetScreenHeight() - g_stUINpcTalk.frmNPCchat->GetHeight() );
		g_stUINpcTalk.frmNPCchat->Refresh();
	}

	if (g_stUINpcTalk.frmNPCchat->GetRight()>=GetRender().GetScreenWidth())
	{
		g_stUINpcTalk.frmNPCchat->SetPos(GetRender().GetScreenWidth() - g_stUINpcTalk.frmNPCchat->GetWidth(), g_stUINpcTalk.frmNPCchat->GetTop());
		g_stUINpcTalk.frmNPCchat->Refresh();
	}
}
