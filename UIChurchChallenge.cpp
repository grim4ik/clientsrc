...
{
	_pNumberBox = g_stUIBox.ShowNumberBox(_evtChallengeNumInput, -1, g_oLangRec.GetString(936), true);	// "�����뾹�ļ۸�"
...
void CChurchChallengeMgr::CheckChallengePrice(void)
{
	if(_pNumberBox)
	{
		...
		{
			char szBuf[256] = {0};
			sprintf(szBuf, g_oLangRec.GetString(940), _nPrice);	// "ȷ�����ļ۸� %d"
			g_stUIBox.ShowSelectBox(_evtChallengeNumCheck, szBuf, true);
		}
		else
		{
			g_pGameApp->MsgBox(g_oLangRec.GetString(937), _stInfo.nCurPrice + _stInfo.nMinbid);//"���õ�����ͼ� %d"
		}
	}
}
