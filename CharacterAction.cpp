...
bool CGameCharacterAction::Init(const _TCHAR *ptcsFileName)
{
	bool		bRet = true;
	...
	while (!feof(fFile))
	{
		...
		StringGet(tcsTemp, 1023, tcsLine, &lIndex," ,\x09\x0a\x0d", 5);
	...
	while (!feof(fFile))
	{
		...
		if (tcsLine[lIndex] == 0x0a || tcsLine[lIndex] == 0x0d) //�س�, ����
			continue;
		if (tcsLine[lIndex] == '/' && tcsLine[lIndex + 1] =='/') // ע��
			continue;
		if (tcsLine[lIndex] =='\t') // tab�� (�ڸ��ı�������"��ɫ���͵�һ�������еĿ�ʼ����)
			continue;
		...
		while (!feof(fFile))
		{
			...
			if (tcsLine[lIndex] == _TEXT('\t')) // tab��
			{
				...
				if (_ttoi(tcsTemp) < 1)
				{
					MessageBox(nullptr, _TEXT(g_oLangRec.GetString(24)), _TEXT(g_oLangRec.GetString(25)), 0);
					goto end;
				}
				//
		...
		while (!feof(fFile))
		{
			...
			if (tcsLine[lIndex] == _TEXT('\t')) // tab��
			{
				...
				// �ؼ�֡
				iCurKeyFrame = 0;
				while (StringGet(tcsTemp, 1023, tcsLine, &lIndex, " ,\x09\x0a\x0d", 5))
				{
					...
					iCurKeyFrame ++;
					StringSkipCompartment(tcsLine, &lIndex, " ,\x09", 3);
				}
			}
			else
			{
...
long StringGet(_TCHAR *out, long out_max,const _TCHAR *in, long *in_from, const _TCHAR *end_list, long end_len)
{
...
void StringSkipCompartment(_TCHAR *in, long *in_from, const _TCHAR *skip_list, long skip_len)
{
    long i;    // temp variable
