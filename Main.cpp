...
// create anti debug thread @mothannakh
DWORD WINAPI ShieldThread(LPVOID);
#include <tlhelp32.h>
//@mothannakh code from https://bit.ly/32GmFcQ //
...
//@mothannakh to check if Cheat eng window opened or not
bool IsCheatRunning(LPCSTR pName) {
	HWND hwnd;
...
*/
    ...
    {
        MessageBox(nullptr, g_oLangRec.GetString(187), "error", MB_OK);
        return 0;
    }
	...
	//mothannakh serial number 
	g_SerialNumber = new GetSerial();
	//serial number end		
...
//	}
	...
	//if(overIP!=-1){
	//	std::string strIP = strParam.substr( overIP + 3, strParam.length() );
		//int nEnd = (int)strIP.length();
		//nEnd = (int)strIP.find( " " );
		//if( nEnd!=-1 ) strIP = strIP.substr( 0, nEnd );
		
		//std::string strList[1];
...
//#endif
	...
	if (g_stUISystem.m_sysProp.m_videoProp.nResolution)
    {
        nWidth  = LARGE_RES_X;
        nHeight = 768;
    }
 ...
 //                             0,0,&ThreadID);
	...
	// anti debug code "@mothannakh// Starting the thread . . .
	DWORD thID;
	CreateThread(nullptr, 0, (LPTHREAD_START_ROUTINE)ShieldThread, 0, 0, &thID);
	// end of thread
    //string str( "Micro" );
    //string rstr( "soft" );
	...
	{
		MessageBox( nullptr, g_oLangRec.GetString(186), g_oLangRec.GetString(185), 0 );
	}
...
//	MessageBox( nullptr, g_oLangRec.GetString(193), "Info", 0 );
}
...
void StdoutRedirect()
{
   ...
   *stdout = *hf;
   int i = setvbuf( stdout, nullptr, _IONBF, 0 );
    ...
    DWORD ThreadID;
    HANDLE hThread = ::CreateThread(nullptr,0,ReadStdout, //�����߳�
                              0,0,&ThreadID);
}
...
// anti debug code "@mothannakh
int checkticks = 0;
DWORD WINAPI ShieldThread(LPVOID) {
	bool IsChecked = true; // toggle it on/off
	while (IsChecked) {
		BOOL bUseFakeServer = IsDebuggerPresent(); // small method to stop game debug in public version @mothannakh
		if (bUseFakeServer) {
			g_ChaExitOnTime.ExitApp();
			// ExitProcess(0);
		}
		// check if cheat enging/wpe running to close the game

		else if (IsProcessRunning("cheatengine-x86_64.exe") || IsCheatRunning("Cheat Engine") || IsCheatRunning("WPE PRO") || IsProcessRunning("wpe_0delay.exe") || IsCheatRunning("x32dbg") || IsCheatRunning("x64dbg")) {

			CGameApp::MsgBoxTime(" A cheat tool or other hacking device has been detected on this system.please close it and try again .. client closing ! ", 20);
			if (checkticks == 20) // close after x time
				g_ChaExitOnTime.ExitApp();
			checkticks++;
		}
		//
