...
//#endif
	...
	//::GetModuleFileName(nullptr,pszAppPath,256);
...
//		_pRenderSurface->BeginScene(surface,nullptr);
//		RenderScene();
...
////
////		HRESULT hr;
////		LPDIRECT3DSURFACE8	pSaveSuf = nullptr;
////		LPDIRECT3DSURFACE8	dsface = nullptr;
////		LPDIRECT3DSURFACE8	surface = nullptr;
////
...
////#ifdef MGR
////		_pCurSuf->GetTex()->GetSurfaceLevel(0,&surface);
////		if(FAILED(hr = m_pDev->SetRenderTarget(surface,nullptr/*_pDSSuf->GetSurface()*/)))
////        {
////            int i = 0;
////        }
////#else
...
//	if(SUCCEEDED(D3DXAssembleShaderFromFile( t_Path, nullptr, 0, &pCode, nullptr )))
//	{
...
void	CAniClock::Render(int x,int y)
{
	...
	CGuiFont::s_Font.BRender(2, buf, x, y, COLOR_GREEN, COLOR_BLACK);
	//
...
void	CLargerMap::RenderScene()
{
	...
	//render npcs icons and player icon on world map @mothannakh 
	static bool RenderNpcIcons = true; //  enable//disable the mode 
	_cSmNpc.Begin();
	if (RenderNpcIcons) {
		...
		for (int i = 0; i < nTotalIndex; ++i) {
			...
			if (p && _tcscmp(p->szMapName, strMapName.c_str()) == 0 && p->NPCType != 0) {
				int Templenw = (_rcWnd.right - _rcWnd.left) / 2;
				int Templenh = (_rcWnd.bottom - _rcWnd.top) / 2;
				Templenw += ((p->dwxPos0 * 100) - _nCenterX) / 125;
				Templenh += ((p->dwyPos0 * 100) - _nCenterY) / 125;
				_cSmNpc.setPos(Templenw, Templenh, _rcWnd.left, _rcWnd.top);
				_cSmNpc.setScaling(NPCSIZE, NPCSIZE); //NPCSIZE
				_cSmNpc.setAngle(180);
				_cSmNpc.setColor(0xffffffff);
				_cSmNpc.setTexture(p->NPCType);
				_cSmNpc.Render();
				//LG("BigMaps", "%s,%d - %d , %d \n", p->szName, p->NPCType, Templenw, Templenh);
			}
		}
	}
	int lenw = (_rcWnd.right - _rcWnd.left) / 2;
	...
	_snprintf_s(psz, sizeof(psz), _TRUNCATE, g_oLangRec.GetString(394), _nCenterX / 100, _nCenterY / 100, fInf);
