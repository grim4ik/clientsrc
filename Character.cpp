...
bool CCharacter::_ShowWings = true;
bool CCharacter::_ShowPets = true;
// �������?
enum eSpecial
{
	enumAlphaIn     = 1,		// ����
...
bool CCharacter::IsOnHideMountMap() const
{
    static vector<string> HideMountMaps =
    {
        "mjing1",
        "mjing2",
        "mjing3",
    ...
    return std::find(HideMountMaps.begin(), HideMountMaps.end(), _pScene->GetCurMapInfo()->szDataName) != HideMountMaps.end();
}


void CCharacter::Render()
{ 
	if (IsMount() && IsOnHideMountMap())
		return;
...
void CCharacter::RefreshUI(int nParam)
{
	...
	//	g_stUIStart.RefreshMainLifeExperience(_Attr.get(ATTR_CLIFEEXP), _Attr.get(ATTR_CLV_LIFEEXP) );
...
bool CCharacter::ChangeReadySkill( int nSkillID )
{
	...
	if( !p )
	{
		g_pGameApp->AddTipText( g_oLangRec.GetString(17), nSkillID);
		return false;
	}
...
void CCharacter::_UpdateHeight()
{
    ...
    if( _nPoseHeightOff==ERROR_POSE_HEIGHT )
    {
		...
		if(GetIsOnMount() && _fMapHeight > 1)	//bar fix 
			_fMapHeight = 1.3f;
...
void CCharacter::UpdateTileColor(int currenthout) {
	BYTE r = 255;
	BYTE g = 255;
	BYTE b = 255;
	if (currenthout >= 0) {
		//now filter shadow depends on our hours 
		if (currenthout == 19 || currenthout == 20)
		{
			r = 255;
			g = 155;
			b = 144;
		}
		else if (currenthout >= 4 && currenthout <= 6) { // early morning bit darky
		 // rbg colors
			r = 169;
			g = 241;
			b = 246;
		...
		else if (currenthout == 22 || currenthout == 23)
		{
			r = 111;
			g = 139;
			b = 232;
...
void CCharacter::setGuildCircle( int colour ,int icon)	{
	s_string mapname = g_pGameApp->GetCurScene()->GetTerrainName();
	if(mapname == "ChaosIcicle" || mapname == "secretgarden")
		return;
	if( _pSideShade ){
		_pSideShade->SetValid( FALSE );
		_pSideShade = nullptr;
...
bool CCharacter::PlayPose( DWORD pose, DWORD type, int time, int fps, bool isBlend )
{
	...
	// 飞�?�用（测试代码）
	if(GetIsFly()){
		if(pose == POSE_WAITING || pose == POSE_WAITING2) pose = POSE_FLY_WAITING;	// 站立 -> 飞�??
		if(pose == POSE_RUN     || pose == POSE_RUN2)     pose = POSE_FLY_RUN;	// 走动 -> 飘走
		if(pose == POSE_SHOW)  pose = POSE_FLY_SHOW;	// 摆酷 -> 空中摆酷
		if(pose == POSE_SEAT)  pose = POSE_FLY_SEAT;	// 坐下 -> 空中�?空坐 
	}
	...
	float velocityMultiplier = 1.0f;
	 ...
	 if ((IsMount() || IsMounted()) && !IsOnHideMountMap() && Is60FPSMode )
	...
	if (!IsHide() && pMount)
	{ 
		pMount->PlayPose(pose, type, fps, isBlend);
		if(!GetCanPkOnMount())
			pose = this->MountPose;
		if (GetCanPkOnMount())
		{
			pMount->SetHide(TRUE);
		}else if(pMount->GetCurPoseType() == POSE_DIE){
			pMount->SetHide(TRUE);
			pose = POSE_DIE;
		} else {
	...
	if( isBlend )
	{
		CCharacterModel::PlayPose(pose, type, time, fps, 1, 5, velocityMultiplier);
	}
	else
	{
		...
		{
			CCharacterModel::PlayPose( pose, type, time, fps, 0, 5, velocityMultiplier);
		}
	}
...
bool CCharacter::UpdataItem( int nItem, DWORD nLink)
{
	...
	case enumEQUIP_MOUNT:
	{
		...
		{
			if (GetPart().SLink[nLink].sID > 0 && this->IsPlayer())
				EquipMount(GetMountInfoFromItemID(nItem));
			else
				RemoveMount();
		}
	...
	case enumEQUIP_FAIRY:{
		...
		if(!GetIsForUI()){		
			...
			if (!g_stUISystem.m_sysProp.m_gameOption.bIsWingShow && _pHandItemEff[enumEQUIP_WING]){
				_pHandItemEff[enumEQUIP_WING]->SetValid( FALSE );
				_pHandItemEff[enumEQUIP_WING] = nullptr;
			} 
			else if (!g_stUISystem.m_sysProp.m_gameOption.bIsFairyShow && _pHandItemEff[enumEQUIP_FAIRY]){
				_pHandItemEff[enumEQUIP_FAIRY]->SetValid( FALSE );
				_pHandItemEff[enumEQUIP_FAIRY] = nullptr;
			}
    ...
    case enumEQUIP_RHAND:
        ...
        if( nItem>0 && nItem!=enumEQUIP_BOTH_HAND ){
			...
			if(_ShowApparel){
				...
				}else if (itemType == enumItemGlow){
					slot = enumEQUIP_GLOWAPP;
				}
...
void CCharacter::SynchroSkillState(stSkillState* pState, int nCount)
{
	...
	if (set.IsTrue(enumChaStateMove) != _ChaState.IsTrue(enumChaStateMove)) {
		if (set.IsTrue(enumChaStateMove))
			...
			if (GetCurPoseType() != POSE_SEAT && !IsMonster()) {
...
void CCharacter::ForceMove( int nTargetX, int nTargetY )
{
	...
	else
	{
		...
		if (g_Render.IsRealFPS()) {
			...
			_fStep = 1.0f / (fDistance / (float)getMoveSpeed() * (float)(fps));
		} else {
...
void CCharacter::SetHide(BOOL bHide)
{
	...
	if (pMount) pMount->SetHide(bHide);	//hide mount using boat
...
{
	if (IsMounted() && !IsOnHideMountMap())
	{
		MPMatrix44 mat;
		pMount->GetRunTimeMatrix(&mat, MountBone);

		if (pMount->GetCurPoseType() == POSE_DIE ) {
			setHeightOff((int)GetDefaultChaInfo()->fHeight); // check if player died or not
		}
		else if (GetCanPkOnMount()) {
			setHeightOff((int)GetDefaultChaInfo()->fHeight);
		}
		else{
			setHeightOff(int((mat._43 * AutoMountHeight + this->MountHeight)));
		}
	...
	{
		MPMatrix44 mat;
		pMountOwner->GetRunTimeMatrix(&mat, mParentBoneID);
		...
		// convert mount scale from mountinfo.txt
		scale.x = pMountOwner->MountScale * 0.01f;
		scale.y = pMountOwner->MountScale * 0.01f;
		scale.z = pMountOwner->MountScale * 0.01f;
		SetScale(scale);
        // apply position with offset and orientation
        // positive MountX shifts mountOwner towards mount's head
        // positive MountY shifts mountOwner towards mount's right shoulder
        float dx = - pMountOwner->MountY * cos(rotZ) - pMountOwner->MountX * sin(rotZ);
        float dy = - pMountOwner->MountY * sin(rotZ) + pMountOwner->MountX * cos(rotZ);
...
void CCharacter::EquipMount(MountInfo* MountInfo)
{
    if (!MountInfo)
        return;
    ...
    int CharID = MountInfo->CharID;
    int BoneID = MountInfo->BoneID[chatype - 1];
    CChaRecord* MountInfoX = GetChaRecordInfo(CharID);
    if (!MountInfoX) CharID = 1;

    if (pMount && CharID != MountID)
    {
        RemoveMount();
    }
    this->MountID = CharID;
    this->MountBone = BoneID;

    if (!pMount)
    {
        //mount pose 
        this->MountPose = MountInfo->PoseID[chatype -1 ];
        //check if mount has custom height/x/y or not 
        this->MountHeight = MountInfo->Height[chatype - 1];
        this->MountX = MountInfo->OffsetX[chatype - 1];
        this->MountY = MountInfo->OffsetY[chatype - 1];
        //Custom height end
        //mount scale 
        MountScale = MountInfo->Scale;
        pMount = _pScene->AddCharacter(MountID);
        pMount->linkTo(this, MountInfo->BoneID[chatype - 1]);
        pMount->setName("");
